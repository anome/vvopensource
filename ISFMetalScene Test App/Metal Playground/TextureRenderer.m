          ...
          onCommandBuffer:(id<MTLCommandBuffer>)commandBuffer
{
    ...
    [self renderFromTexture:offScreenTexture
        extraColorAttachements:@[]
                     inTexture:texture
...
- (void)renderFromTexture:(id<MTLTexture>)offScreenTexture
                inTexture:(id<MTLTexture>)texture
          onCommandBuffer:(id<MTLCommandBuffer>)commandBuffer
              andViewport:(MTLViewport)viewport
{
    [self renderFromTexture:offScreenTexture
        extraColorAttachements:@[]
                     inTexture:texture
...
- (void)renderFromTexture:(id<MTLTexture>)offScreenTexture
    extraColorAttachements:(NSArray<id<MTLTexture>> *)extraTextures
                 inTexture:(id<MTLTexture>)texture
           onCommandBuffer:(id<MTLCommandBuffer>)commandBuffer
{
    const MTLViewport viewport = (MTLViewport){0.0, 0.0, offScreenTexture.width, offScreenTexture.height, -1.0, 1.0};
    [self renderFromTexture:offScreenTexture
        extraColorAttachements:extraTextures
                     inTexture:texture
               onCommandBuffer:commandBuffer
                   andViewPort:viewport];
}
               ...
               andViewPort:(MTLViewport)viewport
{
    ...
    {
        NSLog(@"Render aborted. Extra textures received:(%lu) should be:(%i)", extraTextures.count,
              numberOfExtraColorAttachements);
        return;
    }
    ...
    [renderEncoder setFragmentTexture:offScreenTexture atIndex:MetalBitsTextureIndexZero];

    // Insert extra color attachements
    [extraTextures enumerateObjectsUsingBlock:^(id<MTLTexture> extraTexture, NSUInteger index, BOOL *stop) {
#warning MTO: assumption that MetalBitsTextureIndex enum values & index values are equal
