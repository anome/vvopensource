...
- (MTLViewport)viewportForFrameWidth:(int)frameWidth
...
- (void)drawRect:(NSRect)dirtyRect
{
    @autoreleasepool
    {
        ...
        if( _image != nil )
        {
            ...
            commandBuffer.label = @"MetalImageView commandBuffer";
            const MTLViewport viewport = [self viewportForFrameWidth:(int)_image.width
                                                         frameHeight:(int)_image.height
                                                           viewWidth:self.bounds.size.width
                                                          viewHeight:self.bounds.size.height];
            [textureRenderer renderFromTexture:_image
                                     inTexture:self.currentDrawable.texture
...
- (void)setUnsafeImage:(id<MTLTexture>)unsafeImage
{
    ...
    if( threadSafeImage == nil || threadSafeImage.pixelFormat != unsafeImage.pixelFormat ||
        threadSafeImage.width != unsafeImage.width || threadSafeImage.height != unsafeImage.height )
    {
        NSLog(@"lazy thread safe image init");
        MTLTextureDescriptor *textureDescriptor =
            [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:unsafeImage.pixelFormat
    ...
    [blitCommandEncoder copyFromTexture:unsafeImage
                            sourceSlice:0
                            sourceLevel:0
                           sourceOrigin:MTLOriginMake(0, 0, 0)
                             sourceSize:MTLSizeMake(unsafeImage.width, unsafeImage.height, 1)
                              toTexture:threadSafeImage
                       destinationSlice:0
                       destinationLevel:0
