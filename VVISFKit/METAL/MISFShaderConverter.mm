...
// If not defined, it will use the CLI binary included in the project. Performance is 10+ times slower with the CLI
#define USE_MOLTENVK_LIB
...
+ (NSString *)spirVConvertToMetal:(NSString *)glsl programType:(ProgramType)programType withError:(NSError **)errorPtr
{
    ...
    NSString *glslPath = [FileTools pathForTemporaryFileWithPrefix:@"ZZMOLTENbeforespirv_"];
    [glsl writeToFile:glslPath atomically:YES encoding:NSUTF8StringEncoding error:nil];

    // 2. Run SpirVCross CLI
    NSString *outputFilePath = [FileTools pathForTemporaryFileWithPrefix:@"ZZMOLTENtranspiled_"];

    NSString *programTypeParameter = @"";
    if( programType == ProgramTypeFragment )
    {
        programTypeParameter = @"f";
    }
    else if( programType == ProgramTypeVertex )
    {
        programTypeParameter = @"v";
    }
    ...
    const char *argv[] = {"name",
                          ...
                          "macos",
                          "-t",
                          [programTypeParameter UTF8String],
                          "-mv",
                          "1.1",
                          "-l",
//                          "-p", // Report performance
    ...
    NSString *binaryPath = [frameworkBundle pathForResource:@"MoltenVKShaderConverter" ofType:nil];
    //  -mv 1.1 -gi ./tests/frag.frag -l -p -c -mp macos -mo ./molten.msl
                         ...
                         withError:(NSError **)errorPtr
{
    ...
    case ProgramTypeVertex:
    {
        ...
        if( returnTypeIsVoid )
        {
            ...
            NSString *firstOptionIntermediate =
                [MISFShaderConverter replaceOccurences:@"isf_vertShaderInit();"
                                    ...
                                    withError:(NSError **)errorPtr
{
    ...
    // Spir-V sometimes force inline methods and it doesnt work very well
    intermediate = [intermediate stringByReplacingOccurrencesOfString:@"static inline __attribute__((always_inline))\n"
                                                           withString:@" inline "];
