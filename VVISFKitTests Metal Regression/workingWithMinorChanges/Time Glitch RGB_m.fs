...
// MODIFICATION for SPIRV: two variables with different types were named "lastRow". Renamed lastRow (texture) to lastRowImage
...
void main()
{
	...
	if (PASSINDEX == 0)	{
		vec4		srcPixel = IMG_PIXEL(lastRowImage,vec2(0.5));
	...
	if (PASSINDEX == 1)	{
		vec4		lastRow = IMG_PIXEL(lastRowImage,vec2(0.5));
		if (lastRow.x == 0.0)	{
