...
/*{
	"DESCRIPTION": "ANONYMOUSE by UARALAB Based on http://glslsandbox.com/e#65288.0",
	"CREDIT": "",
	"ISFVSN": "2",
	"CATEGORIES": [
		"XXX"
...
const vec2 ch_start = vec2 (ch_space.x * -6., 2.); // start position
      ...
      vec3 ch_color = vec3 (0.0, 3.0, 5.0);        // character color (R,G,B)
...
{
	ch_color = vec3 (0.0, 10.0, 5.0);  
	vec2 aspect = RENDERSIZE.xy / RENDERSIZE.y;
	uv = ( gl_FragCoord.xy / RENDERSIZE.y ) - aspect / 2.0;
	uv *= 22.0 + sin(TIME*0.3);     //  set zoom size
	uv.x += 5.;
	if (REPEAT_SIGN)
	  uv = -14.0 + mod(8.8*(uv-1.0),ch_space*vec2(26.,6.5));     //  set zoom size
	uv.x += 5.;

	ch_pos = ch_start + vec2(sin(TIME*0.001),0.0);  // set start position

        _ _ _ _ _ _ N O _ s_plus _ C E N S U R A _ _ 
		
	vec3 color = mix(ch_color*(0.6+(sin(uv.x*1.0+.2*TIME)*0.1)), bg_color, 1.0- (0.08 / d));  // shading
