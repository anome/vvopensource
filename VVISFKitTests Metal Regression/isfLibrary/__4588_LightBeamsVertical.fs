...
{ return mat3(cos(phase1*a),1,-sin(phase2*a), 1,1,1, sin(phase2*a),0,cos(phase1*a)) * p; }
...
vec3 raymarch(vec3 ro, vec3 rd)
{
	...
	for( int i=0; i < 12; i++ )
	{
		...
		col= vec3(8.*c*c/hue, c*c, c*c*c/tint) *1.16 + col *0.96; //red
        n -= 1.0;
        if (n<1.0) { break; 
        }
	}
...
void main()
{
    ...
    vec3 uu= normalize(cross(ro, vec3(0.4, .0, 0.0)));
