...
vec2 hash22(vec2 p) { return fract(vec2(271.0, 71.0)*sin(dot(p, vec2(1061.0, 601.0)))); }

void main() 
{
    vec2 uv = (gl_FragCoord.xy - RENDERSIZE.xy*0.5)/RENDERSIZE.y;
    vec2 oP = uv*(8.0-scale) + vec2(0.5, TIME*1.5);
    vec3 bg = vec3(0.05)*vec3(0.8, 0.5, 1.0);
    float pat = clamp(sin((oP.x - oP.y)*twpi*RENDERSIZE.y/5.5) + 0.75, 0.0, 1.0);
    vec2 hoP = hash22(oP);
    bg *= (hoP.x*1.15 + 1.0)*(pat*0.35 + 0.65);
    vec3 col = bg;
    vec4 d = vec4(2e5);
    float dim = 1.0;
    vec2 rndTh[3];
    rndTh[0] = vec2(0.333, 0.667);
    rndTh[1] = vec2(5.667, 0.667);
    rndTh[2] = vec2(.2, 0.667);
    for(int k=0; k<3; k++){
        ...
        if(rnd.x<rndTh[k].x){
            ...
            d.y = abs(max(abs(p.x), abs(p.y)) - 0.5/dim) - lwg/1.0;
            float fo = 4.0/RENDERSIZE.y;
            vec3 pCol = mix(col, vec3(0.8, 0.5, 1.0), 0.5 - pat*0.5)*max(1.0 - d.y/(lwg/2.0), 0.0);
            if (grid) {
                col = mix(col, vec3(2), (1.0 - smoothstep(1.0, fo*3.0, d.y - 3.02))*0.03);
			...
			col = mix(col, vec3(0), 1.0 - smoothstep(0.0, fo, d.x + 2.25/1.4/dim));
			col = mix(col, pCol*0.6*(pat*0.5 + 0.5), 1.0 - smoothstep(0.0, fo, d.x + 0.25/1.4/dim + 1.015));
			col = mix(col, vec3(0), 1.0 - smoothstep(0.0, fo, d.x + 0.375/1.4/dim));
			col = mix(col, bg*0.8, 1.0 - smoothstep(3.0, fo, d.x + 0.375/1.4/dim + 0.015));
            break;
        }
        dim *= 2.0;        
    }    
    //col = mix(col.xzy, col, sign(uv.y)*uv.y*uv.y*1.0 + 0.5);
