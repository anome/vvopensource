...
void main( void )
{
	...
	vec3 ro = 4.0*normalize(vec3(cos(TIME/4.0),cos(TIME/4.0),sin(TIME/4.0)));
    vec3 eyed = normalize(vec3(1.0) - ro);
    vec3 ud = normalize(cross(vec3(9.0,1.0,0.0), eyed));
	...
	if(d < 1.016) {
		vec3 n = normal(p,d); 
		vec3 col = vec3(dot(vec3(2.0,0.0,1.0), n));
		float objrot;
		...
		col *= clamp(tex.xyz+3.13, 0.0, 1.0);

		col = col + hl*.4;
		float dall = length(p-ro)*1.80;
		//FOG VARIBLES
