...
{
    float h = clamp( 0.9+0.5*(b-a)/smoothing, 0.2, 1.0 );
...
{
    p = abs(mod(p, 0.035)-0.0125);
    float x = min(p.x,max(p.y,p.z))/0.0325;
	x *= sin(x*phi+sin(x*phi)*phi)*1.5 + 0.5; 
...
void main()
{
   ...
   // vec3 rd = normalize(vec3(uv, sqrt(1.-dot(uv, uv))*0.5)); // Mild fish lens, if you'd prefer.
    rd.xy *= rot(TIME*0.35);
    rd.xz *= rot(TIME*0.325); // Extra variance.
   ...
   // vec3 ro = vec3(0.7 + TIME*0.125, 0.0, TIME*0.125); // Another lattice traversal cliche.
    ...
    if (dist < FAR)
    {
	   ...
	   // objCol *= bumpSurf3D(sp, sn)*0.5+0.5;
	    ...
	    ld /= lDist;
	    float atten = min( 9.0 /( lDist*0.5 + lDist*lDist*0.1 ), 1.0 ); 
	    float ambient = .25;
	    float diffuse = max(0.0, dot(sn, ld) ); 
	    float specular = max( 0.0, dot( reflect(-ld, sn), -rd) ); 
	    specular = pow(specular, 2.0); 
        float ao = calculateAO(sp, sn)*0.3+0.5;
	    sceneCol = objCol*(vec3(2.9,0.3,3.9)*diffuse + ambient)  + vec3(0.7,0.8,0.9)*specular*0.75;
		sceneCol *= atten*ao;
	}
