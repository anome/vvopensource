...
void main() {
	vec2 uv = (gl_FragCoord.xy * 2.03 -  RENDERSIZE.xy) / RENDERSIZE.x;
	vec2 p = uv * rotate2d(T *1.32); 
	float direction = 220.0/(T/p.x,T/p.y,sqrt(T));
	float speed = T * direction ;
	float distanceFromCenter = cash(direction*dot(uv,p) - length(p));
	p.yx *= rotate2d(-T * 1.80);
	float meteorAngle = atan(p.y, p.x) * (349.0 + cos(atan(speed,pi))+pi);
	...
	float dist = t + adist;
	float meteorDirection = (direction < 0.5) ? -1.5 : 0.0;
	dist = abs(fract(dist) + meteorDirection);
	float lightLength = 23.0/noise(uv.xy);
