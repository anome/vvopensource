...
float eval(in float x, in vec3 p) {
	if ( x < 1.0)
		return abs(max(abs(p.z)-k, abs(p.x)-3.1))-0.01;
	if ( x < 2.0)
...
float map(in vec3 p)
{
	float cutout = dot(abs(p.yz),vec2(1.2))-2.435;

	vec3 z = abs(1.0-mod(p,2.0));
	float d = 499.0;
	float s = 1.0;
	for (float i = 0.0; i <3.0; i++) {
	    z.zx = rotate(z.zx, radians(i*0.0+iGlobalTime));
...
void main()
{
	...
	vec3 cameraOrigin = vec3(iGlobalTime*3.1, 0, 0);

	
	vec2 screenPos = -2.0 + 2.0 * gl_FragCoord.xy / iResolution.xy;
	screenPos.x *= iResolution.x / iResolution.y;
	vec3 rayDir = normalize(vec3(4.0, screenPos));
	
	gl_FragColor = vec4(intersect(cameraOrigin, rayDir), 30.0);
} 