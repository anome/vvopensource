/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https://www.shadertoy.com/view/3dSXRm by evvvvil.  \"Cumulus, the flirt\" - Shader showdown practice session 012.\nLive coded on Twitch. Practicing live on TWITCH every Tuesdays around 21:00 UK time.\nhttps://www.twitch.tv/evvvvil_",
  "INPUTS" : [
    {
      "NAME" : "een",
      "TYPE" : "float",
...
{
  vec2 h,g=vec2(bo(abs(p)-vec3(4.3,0,0),vec3(3.5,Acht,0.4)),3);//Symetry clone couple of boxes
  ...
  h.x=min(bo(p,vec3(30,0.2,Zeven)),h.x);//slow down now it's 6am and im trying compare snooker to international geo politics, it's been a good night
  h.x=min(bo(abs(p)-vec3(6,0,0),vec3(0.2,100.,Zes)),h.x);//Just when you thought the rush was over your mate walks in with DMT from Mexico, ok let me grab my sombrero, at 10am you better keep it stealthy
  ...
  g.x*=Negen;//More definition to avoid artifact. I like my shaders looking sexy, like my wife. Don't call me shallow broh, you would be shallow too if you were handsome.
  return g;
}
...
void main() {
    ...
    uv -= 0.5;uv /= vec2(RENDERSIZE.y / RENDERSIZE.x, 1);//boilerplate code to get uvs in BONZOMATIC live coding software i use.
    tt=mod(TIME*een,50.);//MAin time variable, it's modulo'ed to avoid ugly artifact. Holding time in my hand: playing god is nearly as good as this crystal meth bag
  	b=twee+drie+clamp(sin(tt),-0.5,0.5);//This is just animation variables used in mp or fb
    vec3 ro=vec3(cos(tt*0.2)*vier,cos(tt*0.1)*2.,sin(t*0.3)*4.-vijf),//Ro=ray origin=camera position because everything is relative to a view point, even your ex girlfriend's dubious taste in men
    ...
    {//VOLUMETRIC CLOUDS RENDERING. 
    	cp=ro+rd*(cr+=(0.0-length(cp-ro)*0.00));//This could be fixed step cr+=0.3 or the opposite like this:cr+=(0.1+length(cp-ro)*0.01) if you want the front clouds to be more defined rather than back clouds
        cp.xz*=r2(sin(cp.y*0.0+tt)*.0);//The point of this is to be size coded to be used in 4kb intros and shader showdowns
        if (su.a>0.0||cr>t) break;//so I don't really care how it should be done, unless it's fucking size-coded broh
	...
	gl_FragColor=vec4(pow(mix(su.xyz,co,(1.-su.r)*smoothstep(-1.,1.,length(cp-po))),vec3(.45)),1);
	//gl_FragColor=vec4(pow(mix(co,su.xyz,su.a),vec3(.45)),1); //This would be the simpler more naive way for doing it, without the trick from LJ
}
