...
float RenderScene(vec3 position, out float distance)
{
	...
	float noise = 
	    Turbulence(
	        //position * density + vec3(rateZ, rateX, rateY)*TIME, 
	        position * density,
	        0.1, 
	        1.5, 
	        0.03
	...
	*/
	...
	//vec3 pos = position * density * 0.4 + vec3(rateZ, rateX, rateY)*TIME * 0.1 ;
	            
	vec3 pos = position * 0.5;
	    
	    
	float noise = snoise(pos) * 0.4;
...
vec4 March(vec3 rayOrigin, vec3 rayStep)
{
	...
	return mix(
	    Shade(displacement), // 生成颜色
	    //vec4(0.0, 1.0, 0.0, 1.0),
	    vec4(0.0, 0.0, 0.0, 1.0), // 未命中，show black
	    float(distance >= 0.5) // 与其说 mix，更像个 二选一的 开关
	);
}
...
bool IntersectSphere_2_2(vec3    ro, 
                        vec3    rd, 
                        vec3    pos, // 猜测是 火球 pos 被设为 (0,0,0)
                        float   radius, // 火球外层球 的半径
    ...
    float h = dot(rd,CtoA);
    float c = dot(CtoA,CtoA) - radius*radius;
...
void main(void)
{
	...
	// camera posWS, 人为定义，取任何值都可以，反正 camera 会始终看向 (0,0,0)
	...
	vec4 col = vec4(0.0);
	vec3 origin;
	if(IntersectSphere_2_2(ro, rd, vec3(0.0, 0.0, 0.0), Radius + depth*7.0, origin))
	{
		col = March(origin, rd);
	...
	//float ff = snoise( rd * 77.1 ) * 0.5 + 0.5;
	//col = vec4( ff, ff, ff, 1.0 );
