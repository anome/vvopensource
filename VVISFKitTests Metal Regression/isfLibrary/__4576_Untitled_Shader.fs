...
void main()	{
	...
	vec4        bufferColor = IMG_THIS_PIXEL(bufferVariableNameA);
	//	both of these are the same
	//inputPixelColor = IMG_THIS_PIXEL(inputImage);
	//inputPixelColor = IMG_THIS_PIXEL(bufferVariableNameA);
	...
	gl_FragColor = clamp( mix(inputColor,bufferColor,floatInput),0.,1.);//,floatInput);
}
