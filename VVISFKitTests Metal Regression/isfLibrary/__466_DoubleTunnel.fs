...
//    float radius = forceRadius;
    ...
    for( float i=1.0; i<8.0; i++ )
    {
        ...
        if (d != 0.0) force = force + (ColorfulZoom/(70.0*d))*d1*h.z;
    }
    
    vec3 col;
    col.r = 1.0-(abs(force.x*(SuckIn-2.0)));
    col.g = 1.0-(abs(force.y*1.5)*abs(force.x*1.5)*ColorfulZoom*abs(1.0-p.y));
    col.b = 1.0-(abs(force.y*1.5));
