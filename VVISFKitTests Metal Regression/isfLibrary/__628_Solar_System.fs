...
void main()
{
	...
	for (int i = 0; i < 100; i++)
	{
		...
		v1 += dot(p,1./p)*.0015 * (1.8+sin(length(uv.xy*13.0)+.5-TIME*.2));
		v2 += dot(p,1./p)*.0015 * (1.5+sin(length(uv.xy*13.5)+2.2-TIME*.3));
		c = length(p.xy*.5) * 0.35;
	}
	...
	vec3 col = vec3(re, gr, bl) + smoothstep(0.015, .0, len) * .9;
