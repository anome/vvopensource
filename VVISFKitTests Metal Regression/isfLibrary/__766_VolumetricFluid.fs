...
const int _VolumeSteps = 10;
const float _StepSize = 0.2; 
const float _Density = 0.09;

const float _SphereRadius = 3.0;
const float _NoiseFreq = 1.1;
const float _NoiseAmp = 3.0;
const vec3 _NoiseAnim = vec3(0., 1.0, 1.0);
const vec2 uSize = vec2(1.0);
...
float hash( float n )
{
    return fract(cos(n*0.0001231)*413341.5453);
}
...
{ 
  float d = length(p-vec3(0.5, 0.5, 0.5)) - _SphereRadius;  // distance to sphere
...
void main(void)
{
    vec2 p = (vv_FragNormCoord.xy)*2.0-1.0;
