...
void main() {
	...
	for (float i=0.0; i<4.0; ++i)	{
		...
		float		imageChannelAmount = renderDotImageColorCMYK[int(i)];
		...
		if (fragDistanceToDotCenter < dotRadius)	{
			cmykAmounts[int(i)] += smoothstep(dotRadius, dotRadius-(dotRadius*smoothing), fragDistanceToDotCenter);
		}
		...
		for (float j=0.0; j<4.0; ++j)	{
			...
			imageChannelAmount = renderDotImageColorCMYK[int(i)];
