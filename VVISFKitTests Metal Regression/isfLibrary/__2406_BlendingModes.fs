/*{
  ...
  "DESCRIPTION" : "Automatically converted from https://www.shadertoy.com/view/XdS3RW by ben.  Photoshop layer blending modes.",
  "INPUTS" : [
    {
      "NAME" : "inputImage",
      "TYPE" : "image"
...
void main() {
	...
	vec2 uv = gl_FragCoord.xy / RENDERSIZE.xy * vec2(1.0,-1.0) + vec2(0.0, 1.0);
	int id = int(floor(uv.x * 5.0)) + int(floor(uv.y * 5.0))*5;
	
	// source texture (upper layer)
	vec3 s = IMG_PIXEL(inputImage, gl_FragCoord.xy).xyz;
	
	// destination texture (lower layer)
	vec3 d = IMG_PIXEL(inputImage2, gl_FragCoord.xy).xyz;
