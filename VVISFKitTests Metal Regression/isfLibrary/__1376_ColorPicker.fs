...
// ColorPicker by mojovideotech
// Auto-converted from:
// glslsandbox.com/e#30493.1
// Simple color picker... now with mouse control
// public domain
...
#define goTYPE p = ( gl_FragCoord.xy /RENDERSIZE.xy ) * vec2(64,32);vec3 c = vec3(0);vec2 cpos = vec2(2.*sin(TIME*1.1001234567890),1.+0.25*abs(sin(TIME*2.)));vec3 txColor = vec3(1);
...
vec4 mem_load(){
	...
	Read saved input. 
	*/
	return IMG_NORM_PIXEL(bb,mod(vec2(0.),1.0));
}
void mem_save(){
	/*
	...
	*/
	if(length(gl_FragCoord.xy) < 1.){
		...
		vec2 m = (mouse-0.5)* vec2(1.0, RENDERSIZE.y/RENDERSIZE.x);
		if(length(m) > 0.15 && length(m) < 0.2){
			gl_FragColor.x = fract(0.25-atan(m.x, m.y)/(2.*PI));
		}
		if(max(abs(m.x), abs(m.y)) < 1./12.){
			gl_FragColor.yz = 0.5+m*6.;
		}
		//gl_FragColor = vec4(fract(TIME/2.));
	}
}
...
void main()
{
    ...
    vec2 p = (gl_FragCoord.xy/RENDERSIZE*2.0-1.0) * vec2(1.0, RENDERSIZE.y/RENDERSIZE.x);
    ...
    color = smoothstep(0.0, -2.0/RENDERSIZE.y, sd)*color;
    color.rgb = mix(vec3(1.0), color, smoothstep(-2.0/RENDERSIZE.y, 0.0, hsd));
    color.rgb = mix(vec3(1.0), color, smoothstep(-2.0/RENDERSIZE.y, 0.0, ssd));
    color.rgb = mix(selectedColor, color, smoothstep(-2.0/RENDERSIZE.y, 0.0, ssd_inner));
    float mask = smoothstep(2.0/RENDERSIZE.y, 0.0, -sd);
    gl_FragColor = vec4(color, 1.0-mask);
	
	if(length(p) > 0.420){
		gl_FragColor = vec4(selectedColor,1.)*pow(length(p)-0.420, 0.5);
	}
