...
void main() {
	float multi = 4.;
	float T = rate1 * pi * multi * 7.;
	float TT = rate1 * pi * multi * 7.;
	//float TT = T;
	vec2 p=(2.*isf_FragNormCoord);
	for(int i=1;i<7;i++) {
    	vec2 newp=p;
		float ii = float(i);  
    	newp.x+=depthX/ii*sin(ii*pi*p.y+T*nudge+cos((TT/(multi*ii))*ii));
    	newp.y+=depthY/ii*cos(ii*pi*p.x+TT+nudge+sin((T/(multi*ii))*ii));
