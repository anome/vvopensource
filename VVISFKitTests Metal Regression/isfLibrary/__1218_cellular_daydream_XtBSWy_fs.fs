/*
{
  ...
  "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/XtBSWy by gtoledo3.  Combining cellular noise with various uv warp effects, zoom/density levels, and color mapping, for creative effect.",
  "IMPORTED": [
...
void main(){
	...
	vec2 position = gl_FragCoord.xy / RENDERSIZE.x - offset;
	...
	float rand = mod(fract(sin(dot(gl_FragCoord.xy / RENDERSIZE.xy, vec2(12.9898,1980.223+time))) * 43758.5453), .05);
	...
	l=min(l,l2);
    //vignette
	float v=length(gl_FragCoord.xy / RENDERSIZE.xy-.5);
    v=smoothstep(v,.8,.7);
	gl_FragColor=vec4(hsv((time*.01)+.6-sin(l*1.8)*2.9,l*.65,l-.2),1.);
	gl_FragColor.rgb +=vec3(rand);
    gl_FragColor.rgb -=vec3(1.-v);
}
