...
bool intersects(vec3 ro, vec3 rd, vec3 box_center, float box_size, out float t_intersection)
{
    ...
    if (t_near > t_far || t_far < 2.0) return false;
    t_intersection = t_near;
    return true;
}

mat3 camera(vec3 e, vec3 la) {
    vec3 roll = vec3(2, 1, 4);
...
void main(void)
{
	const float INFINITY = 1e3,cluster_size = 3.0, EPSILON = .01;
    ...
    vec3 rd = camera(ro, vec3(0))*normalize(vec3(uv, 5.));
    for (float i = 0.; i < cluster_size; i++) {
        for (float j = 0.; j < cluster_size; j++) {
            for (float k = 0.; k < cluster_size; k++) {
                vec3 p = 4.0*(vec3(i, j, k) - 0.5*vec3(cluster_size-3.0,cluster_size,cluster_size-2.0));
				float l = length(p), s = .1 + .6*pow(.5 + .5*sin(rate*rate*TIME*phi - j*l),k+0.9), t = 2.;
    ...
    if (t_intersection == INFINITY)
        c = mix(vec4(.0, .6, .5, 1.), vec4(0.9, 0., 3., 0.5), 0.25*length(uv));
    else
        c = inside*vec4(1.0, sin(TIME), 0.0, 1.);
