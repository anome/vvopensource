...
{ return mat3(cos(phase1*a),0,-sin(phase2*a), 1,1,1, sin(phase2*a),0,cos(phase1*a)) * p; }
...
{
	float t=morph;
	vec3 col=vec3(1); float c=0.;
	float n = floor(mult);
	for( int i=0; i < 12; i++ )
	{
		t+= exp(c*-2.0) *density;
		c= map(t *rd +ro);               
		col= vec3(9.*c*c, hue*hue*c*c, c*c*c) *0.16 + col *0.96; //green
		col= vec3(8.*c*c, c*c, tint*tint*c*c) *0.30 + col *0.96; //blue
...
void main()
{
    ...
    vec3 ro= roty(vec3(2.), TIME*rate + mouse.x);
    vec3 uu= normalize(cross(ro, vec3(1.4, .0, 0.0)));
    vec3 vv= normalize(cross(uu, ro));
    vec3 rd= normalize(p.x*uu + p.y*vv - ro*1.5 );
    gl_FragColor.rgb= log(raymarch(ro,rd) +0.0) *0.5;
    gl_FragColor.a= 3.0;
}