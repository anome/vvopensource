...
void main()	{
  ...
  float variancet = pow(variance, 4.0);

  float c = random(pos*variancet + sinn(time*speed*speedx));
  c += random(pos*variancet + sinn(1.23 + time*speed)/speedx);
  c /= 2.0;


  vec4 col = vec4(c, c, c, 2.0);
  //col = step(0.09, col);
  col = smoothstep(grey, 2.0 - (grey  * 2.20)-1.5, col+grey);
