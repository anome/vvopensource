...
/*{
	"DESCRIPTION": "EsDictadura by Uará Lab, about the situation in chile, 2020. Forked from http://glslsandbox.com/e#65321.0",
	"CREDIT": "",
	"ISFVSN": "2",
	"CATEGORIES": [
		"XXX"
...
const vec2 ch_size  = vec2(5.5, 0.0) * 0.0;              // character size (X,Y)
const vec2 ch_space = ch_size + vec2(1.3, 2.0);    // character distance Vector(X,Y)
const vec2 ch_start = vec2 (ch_space.x * -9.2, -2.); // start position
...
vec3 hsv2rgb_smooth( in vec3 c )
{
	...
	rgb = rgb*rgb*(3.0-3.0*rgb); // cubic smoothing	
...
void main() 
{
    ...
    vec3 ch_color = vec3(0.0);
    if(colorMode == 0){
    ch_color = hsv2rgb_smooth(vec3(TIME*fxSpeed+uv.y*fxHue,fxSaturation,fxBright));
    } else if(colorMode == 1){
    ch_color = hsv2rgb_smooth(vec3(fxHue,fxSaturation,fxBright));
    } 
    
	vec3 bg_color = vec3(_d*backgroundR, _d*backgroundG, _d*backgroundB);
	uv.x += 0.2+sin(TIME+uv.y*0.2)*0.2;
	ch_pos = ch_start;

	    _ _ _ F E L I Z _ _ _ _ _ _ _ _ nl1;
		_ _ _ N A V I D A D _ _ _ _ _ _ nl2;
		_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ nl3;
		_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ 
			 
                 
		
	vec3 color = mix(ch_color, bg_color, 1.0- (0.08 / d*strokeWidth));  // shading
