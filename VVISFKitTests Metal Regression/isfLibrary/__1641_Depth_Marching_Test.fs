...
// Camera uniforms
const vec3 g_camUp = vec3(0.0, 1.0, 0.0);
vec3 g_camRight = vec3(cos(cameraRotateY), 0.0, sin(cameraRotateY));
const vec3 g_camForward = vec3(0.0, 0.0, 1.0);
const vec3 g_eye = vec3(0.0, 0.0, -1.0);

// Light uniforms
const vec3 g_light0Position = vec3(1.0);
const vec4 g_light0Color = vec4(0.5, 0.0, 1.0, 1.0);
...
void main(void) {
	vec2 uv = vv_FragNormCoord * 2.0 - 1.0;
	...
	// 4xAA
	// vec2 hps = vec2(1.0) / (RENDERSIZE * 2.0);
