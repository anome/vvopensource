...
{
	float steps = 1. + mod(floor(t),5.0*nnXX); // pattern cycling
	t = mod(t, 1.0);
	...
	angle = mod( angle + PI * 3.0, PI * 0.777);
	
	float dist = length(p);
	
	float ring = floor(pow(dist*1.5+mod(nnXX,angle),.5) * num_rings - t); // tweak!
	...
	float tAngle =  PI * 2.0 * mod(ringTime * ring*2.0, 1.0);
