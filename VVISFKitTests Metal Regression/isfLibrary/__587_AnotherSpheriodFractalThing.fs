...
vec3 spin(vec3 v) {
    for(int i = 1; i <5; i++) {
		...
		v=(vec3(sin(v.x+CX/pi),sin(v.y+CY/pi),sin(v.z))*delicateORviolent+0.55).yzx;
    }
    return (v.xyz);
}
...
vec3 F(vec2 E, float G) {
	...
	return vec3(.5+.75*(cos(d)/1.2),.3+.67*cos(d-TIME*cycle),.1+.95*cos(G));
}
...
gl_FragColor=vec4(spin(0.1-F(H.zx,H.y)/3.),1.0);
}
