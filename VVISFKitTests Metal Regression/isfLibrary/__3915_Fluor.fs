...
float fCrystalShard(vec3 p, float size) {
    float d;
    float width = size * (Four + Five)+Seven;
    vec3 o = normalize(vec3(1,0,-.04));

    pModPolar(p.xy, 5.*Ten);
    float part1, part2;
...
float fCrystal(vec3 p, float id, float focus) {
    ...
    float size2 = cos(time * TAU * 2. + focus * 5. + id) * .5 + Eight;
    
    size = alias(size, Nine);
    size2 = alias(size2, Nine);
    
    float height = size2 * .1 + .5;
    float offset = .9;
    float d;
