...
vec3 colorSlit5(vec2 pt, float param) {
    ...
    pt = pt * rInv - vec2(rInv+mod(u_time,5998.824),0.0);
...
vec4 bleeding()
{
    ...
    for (float i = 0.; i<iterations; i+=1.)
    {
        ...
        norm += bleedcurve (i/iterations) * 2. 
                + (noise1( st+sin(u_time*9.1)).x - 1. /*not -.5 because we use pow below*/) * 0.05; //adds grain
...
vec4 glitch1(){
	...
	float time = mod(u_time, 2.512); // + modelmat[0].x + modelmat[0].z;
...
vec4 glitch3_windfilter(){
	...
	// Set up offset
	vec2 offsetRedUV = uv;
	offsetRedUV.x = uv.x + rand(vec2(u_time*0.03,uv.y*0.42)) * 0.001;
	offsetRedUV.x += sin(rand(vec2(u_time*0.006, uv.y))*0.5)*magnitude;
	
	vec2 offsetGreenUV = uv;
	offsetGreenUV.x = uv.x + rand(vec2(u_time*0.004,uv.y*0.002)) * 0.004;
	offsetGreenUV.x += sin(u_time*0.0001)*magnitude;
	
	vec2 offsetBlueUV = uv;
	offsetBlueUV.x = uv.y;
	offsetBlueUV.x += rand(vec2(cos(u_time*0.01),sin(uv.y)));
