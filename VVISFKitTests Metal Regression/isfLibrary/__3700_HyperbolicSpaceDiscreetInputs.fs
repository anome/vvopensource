...
////////////////////////////////////////////////////////////
// HyperbolicSpace  by mojovideotech
//
// from :
// glslsandbox.com/\e#1479.2  by Kabuto.
//
...
#define 	a	1.61803398874989484820	// (sqrt(5)+1)/2
#define 	b	2.05817102727149225032	// sqrt(2+sqrt(5))
#define 	c	1.27201964951406896425	// sqrt((sqrt(5)+1)/2)
#define 	d	2.61803398874989484820	// (sqrt(5)+3)/2
...
void main( void ) {
	...
	float tcos, tsin;
	/*tcos = cos((inputy-.5)*2.1);
	tsin = sin(-(inputy-.5)*2.1);
	dir *= mat3(1,0,0,0,tcos,-tsin,0,tsin,tcos);*/
	tcos = cos((inputx-.1)*4.1);
	tsin = sin(-(inputx-.1)*4.1);
	...
	float psinh, pcosh;
	mat4 movemat;
	float paramlimit = max(.5,length(inputx-.5));
	...
	for (int i = 0; i < 14; i++) {
		...
		if (rad2 < (1.-inputy)*.15-.05 && (i > 0 || dot(pos,dir) < 0.)) { dist += length(pos); break; }
		...
		if (edge < (inputy-.6)*.1 || edge > .16+inputy || dist > maxdist) { break;}
		dir = hreflect(pos, dir);
	}
	float lq = min(1.,1./min(dist, maxdist) - 1./maxdist);
	lq = 1.-1./(1.+lq*2.);
	//lq = 1.-1./(1.+lq);
	gl_FragColor = vec4(pow(lq,color1), pow(lq,color2), pow(lq,color3)-pow(lq,4.)*(1.-lq), 1.0 )*1.;
}