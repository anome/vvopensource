...
/////////////////////////////////////////////////////////////
// EquiRec_SpaceIsThePlace  by mojovideotech
//
...
void main() {
	...
	vec2 st = gl_FragCoord.xy / RENDERSIZE.xy;
		...
		vec2 uv = vec2(sin(th) * cos(ph), cos(th));
		vec3 g = vec3(sin(th) * cos(ph), sin(th) * sin(ph), cos(th));

//	vec2 uv=gl_FragCoord.xy/RENDERSIZE.xy-.5;
// 	uv.y*=RENDERSIZE.y/RENDERSIZE.x;
	vec3 p = vec3(st / 2.0, 0.0) + vec3(g);
	p.x -= 0.25*move.x;
	p.y -= 0.25*move.y;
	...
	p.xy *= -rot1;
	p.yz *= rot2;
	p = abs(vec3(density)-mod(g,vec3(density*2.)));
	float t = field(g);
	float v2 = (1. - exp((abs(g.x) - 1.) * 6.)) * (1. - exp((abs(g.y) - 1.) * 6.));
