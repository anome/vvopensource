...
float Parabola( float x, float n ) { return pow( 3.0*x*(2.0-x), n ); }

void main()
{
	vec2 pos = gl_FragCoord.xy/RENDERSIZE.xy;
	if (mirrorV) { if (pos.y < 0.0) pos.y = 0.0-pos.y; }
	if (mirrorH) { if (pos.x < 0.0) pos.x =0.0-pos.x; }
	pos.x *= RENDERSIZE.x / RENDERSIZE.y;
	if (flipH) { pos.x = 2.0 - pos.x; }
