...
float L(vec2 p, vec2 a, vec2 b) {
	...
	d = length(a-b);
	float f = S(1.0, fade, d);
	m *= f;
	m += m*S(0.05, 0.06, abs(d - 0.5)) * 0.0;
	return m;
}
	...
	for(float y=-1.0; y<=2.0; y++) {
		for(float x=-1.0; x<=2.0; x++) {
			...
			if(mathOp == 0){
			    ...
			    f *= dot( sin(N21(id+offs) * 6.2831 + (flash*TIME)) * 0.4 + 0.6, flash);
			} else if(mathOp == 2){
			    f *= dot( fract(N21(id+offs) * 6.2831 + (flash*TIME)) * 0.4 + 0.6, flash);
			}
			m += f;
		}
	}
