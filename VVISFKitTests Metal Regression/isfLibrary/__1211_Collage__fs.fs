...
/*
		float r = rand(xy);
		float d = floor(region*depth)*r;
		
		if (d > translated*floor(rand(zw))*depth) {
		} else xy = zw;
		*/
		xy.x = mix(zw.x,(xy.x - floor(xy.x / CellWidth)*CellWidth) + CellWidth * floor((translated / rows)*abs(depth)),depth);
		//xy.x = (xy.x - floor(xy.x / CellWidth)*CellWidth);
		xy.y = mix(zw.y,xy.y - floor(xy.y / CellHeight)*CellHeight + CellHeight * floor(mod(translated , cols)*abs(depth)),depth);
