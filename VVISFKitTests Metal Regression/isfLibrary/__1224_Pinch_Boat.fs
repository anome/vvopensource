...
vec3 blur(vec2 uv, vec2 coords)
{
	...
	col += IMG_NORM_PIXEL(iChannel0, vec2(X1, Y1))*0.1;
	col += IMG_NORM_PIXEL(iChannel0, vec2(X2, Y2))*0.1;
	col += IMG_NORM_PIXEL(iChannel0, vec2(X1, Y2))*0.1;
	col += IMG_NORM_PIXEL(iChannel0, vec2(X2, Y1))*0.1;
	
	col += IMG_NORM_PIXEL(iChannel0, vec2(invX1, invY1))*0.15;
	col += IMG_NORM_PIXEL(iChannel0, vec2(invX2, invY2))*0.15;
	col += IMG_NORM_PIXEL(iChannel0, vec2(invX1, invY2))*0.15;
	col += IMG_NORM_PIXEL(iChannel0, vec2(invX2, invY1))*0.15;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	inputDistort.r = IMG_NORM_PIXEL(iChannel0,rCoords).r;
	inputDistort.g = IMG_NORM_PIXEL(iChannel0,gCoords).g;
	inputDistort.b = IMG_NORM_PIXEL(iChannel0,bCoords).b;
