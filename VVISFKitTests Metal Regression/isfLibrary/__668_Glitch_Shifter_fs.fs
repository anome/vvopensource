...
void main()
{
	...
	float	randSeed = (random_seed == 0.0) ? TIME : random_seed;
	...
	if (randomize_size)	{
		random_offset = mod(rand(vec2(randSeed,randSeed)), 1.0);
	...
	if ((shifted) && (rand(vec2(randSeed, shift.x * shift.y)) < use_alt_image))
	...
	if (composition_mode == 5)	{
		comp_mode = int(floor(6.0 * rand(vec2(randSeed, shift.x + shift.y))));
	}
	if (comp_mode == 1)	{
