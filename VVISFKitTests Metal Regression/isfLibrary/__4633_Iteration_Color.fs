...
void main() {
	...
	vec3 finalColor = vec3(0.6, 0.1, 0.9);
	if (invert) { 
		color = invertColor(color);
	}
    finalColor.r += color.x + color.y + color.z;
	finalColor.g += color.x + color.y - color.z;
	finalColor.b += color.x - color.y - color.z;
