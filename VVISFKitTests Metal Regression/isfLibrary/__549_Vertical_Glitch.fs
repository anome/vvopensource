...
void main() {
  int bar = int(isf_FragNormCoord.x*RENDERSIZE.x) / int(barWidth);
  float scale = 1.0 + pos(bar) * mix(0.0, -10.0, ease1(amplitude));
  float phase = ease1(progress) * scale;
  float posY = isf_FragNormCoord.y;
  vec4 c;

  vec2 p = vec2(isf_FragNormCoord.x, isf_FragNormCoord.y + mix(0.0, 1.0, phase));
