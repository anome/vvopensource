...
vec2 doModel(vec3 p) {
	...
	vec2 res = vec2( sdSphere(   p - vec3(  .0, sin(TIME * 0.34)*0.3 + .5,  .0 ), 0.25 )                                , mID1 );
	res = opU( res, vec2( sdBox(      p - vec3( 1.0, sin(TIME * 0.14)*0.3 + .5,   .0 ), vec3( .25 ))                     , mID1 ));
	res = opU( res, vec2( udRoundBox( p - vec3( 1.0, sin(TIME * 0.64)*0.3 + .5,  1.0 ), vec3( .15 ), 0.1 )               , mID1 ));
	res = opU( res, vec2( sdTorus(    p - vec3(  .0, sin(TIME * 0.344)*0.3 + .5, 1.0 ), vec2( 0.20, 0.05 ))              , mID2 ));
	res = opU( res, vec2( sdCapsule(  p,  vec3(-1.3, sin(TIME * 0.754)*0.3 + .5,  -.1 ), vec3( -1.0, 0.20, 0.2), 0.1 )    , mID1 ));
	res = opU( res, vec2( sdTriPrism( p - vec3(-1.0, sin(TIME * 0.234)*0.3 + .5, -1.0 ), vec2(0.25,0.05) )                , mID1 ));
	res = opU( res, vec2( sdCappedCylinder( p - vec3( 1.0, sin(TIME * 0.54324)*0.3 + .5, -1.0 ), vec2(0.1,0.2) )                  , mID3 ));
	res = opU( res, vec2( sdCone(     p - vec3( 0.0, sin(TIME * 0.2344)*0.3 + .5, -1.0 ), vec3( 0.8, 0.6, 0.3 ))           , mID3 ));
	res = opU( res, vec2( sdHexPrism( p - vec3(-1.0, sin(TIME * 0.534)*0.3 + .5,  1.0 ), vec2( 0.25, 0.05 ))              , mID1 ));
...
vec3 material( float materialID  ){
	// return sin( vec3(1.0,0.2,0.5) * ( materialID * 10.0 ) ) * 0.5 + 0.5;
    return vec3(0.9,0.002,0.001);
}
...
vec3 renderRays(){
	...
	if ( res.y > -0.5 ) {
		...
		color = material( res.y ) * lighting( pos, nor, ro, rd );
...
vec3 Blur(vec2 uv, float radius)
{
	...
	for (int i = 0; i < ITERATIONS; i++)
    {
        ...
        acc += texture2D( blurBuffer, uv + circle * Sample(random), radius * 10.0 ).xyz;
    }
...
void main() {
    ...
    // float grainF = mix(0.5, grain( fract(uv + TIME*200.), RENDERSIZE.xy/2.3), 0.3) - 0.5;
    // rays += vec3(grainF);

    if(PASSINDEX == 0){
        vec3 rays = renderRays(); 
        Blur(gl_FragCoord.xy, 2.0);
	    gl_FragColor.rgb = rays;
    }
    if(PASSINDEX == 1){
        vec4 pass0 = IMG_PIXEL(blurBuffer, gl_FragCoord.xy);

	    // gl_FragColor.rgb = Blur(uv, 2.0);
