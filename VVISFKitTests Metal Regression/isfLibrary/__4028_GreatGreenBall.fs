...
#define Radius 1.5
#define NoiseSteps 4
#define Color1 vec4(0.0, 0.0, 1.0, 1.0)
#define Color2 vec4(0.0, 0.9, 0.9, 1.0)
#define Color3 vec4(0.0, 0.5, 1.0, 1.0)
#define Color4 vec4(0.2, 0.0, 0.3, 1.0)
...
void main(void)
{
	...
	vec3 uu = normalize(cross( vec3(0.0, 1.0, 1.0
	), ww));
	vec3 vv = normalize(cross(ww, uu));
	vec3 rd = normalize(p.x*uu + p.y*vv + 1.5*ww);
	vec4 col = vec4(0.0, 0.3, 0.9, 1.0);
	vec3 origin;
	if(IntersectSphere(ro, rd, vec3(0.0), Radius + depth*2.0, origin))
	{
		col = March(origin, rd);
	}
