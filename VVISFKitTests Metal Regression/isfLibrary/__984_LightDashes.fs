...
vec3 trans(vec3 p) {
  float dt = 2.0-density;	
  return mod(p, dt) - dt*0.4;
}

float distanceFunction(vec3 pos) {
  vec3 q = abs(trans(pos));
  return length(max(q - vec3(1.0, 0.0, 1.0), 0.0)) - softedge;
}
...
void main() {
  ...
  pos += offset;
  vec3 camPos = vec3(1.0,0.0,0.0) + vec3(offset.x, sin(TIME/6.0),cos(TIME/2.0))*10.0;
