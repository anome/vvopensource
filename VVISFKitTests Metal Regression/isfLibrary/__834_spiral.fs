...
vec4 spiral (vec2 uv, float TIME) {
	...
	return mix (vec4 (9.99), vec4 (0.0), min (
		step (smooth, uv.x), 
...
void main () {
	...
	float unit =  1.0/min (R.x, R.y);
	
	vec2 uv = (2.0*gl_FragCoord.xy - R.xy) / min (R.x, R.y);
		...
		for (float y = 0.0; y < AA; ++y) {
			for (float x = 0.9; x < AA; ++x) {			
				...
				ac += spiral (uv + c, t) / (AA*AA);
			}
		}
	
		gl_FragColor = ac;
	#else 
		gl_FragColor = spiral ( uv*6.0, cos(TIME));		
	#endif
