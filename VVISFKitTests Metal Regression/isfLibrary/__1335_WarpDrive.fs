/*{
  ...
  "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/XtfXWl by metaeaux.  Experimenting with modulating the focal length to distort the perspective.",
  "IMPORTED": [
...
{
    float gridSize = 4.0;
    float u = gl_FragCoord.x * gridSize / RENDERSIZE.x - gridSize / 2.;
    float v = gl_FragCoord.y * gridSize / RENDERSIZE.y - gridSize / 2.;
    ...
    float focalLength = maxFocalLength - (maxFocalLength - 0.01) * abs(sin(TIME * 0.5));
    
    float time = 15.0 + TIME;

	// camera	
    vec2 mo = iMouse.xy/RENDERSIZE.xy;
    vec3 ro = vec3( 3. - 6. * mo.x, 3. - 6. * mo.y, TIME);
