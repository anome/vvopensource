...
void main()
{
	if (PASSINDEX == 0)	{
		...
		float		index = (gray(inputColor) * float(colorCount));	
		//float		index = (vv_FragNormCoord.x * float(colorCount));
		...
		if (colorMode == 0)	{
			if (mod(index, 2.0) >= 1.0)	{
				...
				outColor.r = outColor.r - (outColor.r);
			}
		
			outColor.g = outColor.g - variation * (index / 2.0);
		
			if (outColor.g < 0.1)	{
				outColor.g = outColor.g + variation * (index / 2.0);
				outColor.g = outColor.g - (outColor.g);
			}
		
			outColor.b = outColor.b - variation * (index / 4.0);
			if (outColor.b < 0.2)	{
				outColor.b = outColor.b + variation * (index / 4.0);
				outColor.b = outColor.b - (outColor.b);
			}
		}
		//	Split complimentary – saturation and brightness variations on a 3 fixed 120 degree hues
			...
			outColor.b = outColor.b - variation * (index / (divisor));
			if (outColor.b < 0.1)	{
				outColor.b = outColor.b + variation * (index / (divisor));
				outColor.b = outColor.b - (outColor.b);
			}
