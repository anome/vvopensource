...
{
    return AddTheseTwoShapes(AddTheseTwoShapes(Box(pos, vec3(side_1_h,side_1_w,0.5)),Box(pos, vec3(0.5,0.5,side_3_h))),Box(pos, vec3(0.5,side_2_h,0.5))) * Slider3;
}
...
for (int i = 0; i < MAX_ITER; i++)
{
    ...
    totalDist += dist * Slider1;
    pos += dist * rayDir * Slider2; // Advance the point forwards in the ray direction by the distance
}
    if (dist < EPSILON)
{
    // Lighting code
