/*{
  "CATEGORIES": [
    "raymarching","reflection","refraction","Automatically Converted"],
  "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/XsXSDl by EvilRyu.  A light bulb",
  "IMPORTED": [
    {
      "NAME": "iChannel1",
      "PATH": [
...
vec3 getcolor(vec3 ro, vec3 rd, out vec3 pos, out vec4 mate, out vec3 n, 
    ...
    type = res.y;
    col += pow(max(1.0 - res.z,0.0), 21.0+20.0*sin(TIME))*
           (vec3(1.7, 0.6470, 0.0));
    if(res.x > 0.0){
        pos = ro + rd * res.x;
...
vec3 tracer(vec2 fragCoord) {
    float fov=3.0;
    vec3 col = vec3(0.0);
    vec2 p = 2.0*(fragCoord.xy)/RENDERSIZE.xy-1.0;
    p.x*=RENDERSIZE.x/RENDERSIZE.y;
	vec2 m = iMouse.xy / RENDERSIZE.xy;
	if (iMouse.x == 0.0 && iMouse.y == 0.0) {
		m = vec2(TIME * 0.06 + 0.14,0.5);	
	}
	m = -1.0 + 2.0 * m;
	m *= vec2(4.0,-1.5);
...
void main()
{
	...
	vec3 col = tracer(gl_FragCoord.xy);
	// post
