...
// Mandelbroken1 by mojovideotech
// based on :
//---------------------------------------------------------
...
#define FIELD 6.
#define ITERATION 15
#define CHANNELA bvec3(false,true,false)
#define CHANNELB bvec3(true,false,false)
#define CHANNELC bvec3(false,false,true)
#define PI4 0.7853981633974483
#define TONE vec3(-0.5,-0.67,-0.5)
...
float mandelbrotEQ(vec3 c,float t)
{
	...
	for(int i=0; i<ITERATION; ++i)
	{
		...
		zi.y = 2.*sin(z.x*z.y);
		zi.z = float ((z.z/TIME)/(z.x*z.y) + c);
		zi.xyz += c;
		if(dot(z.xy,z.xy)>6.0)break;
		z.w++;
		z.xyz=zi;
	}
	z.w /= float(ITERATION);
...
//---------------------------------------------------------
vec3 computeColorA(float fv)
{
	vec3 color = vec3(vec3(CHANNELA)+TONE);
	color -= (fv);
//	color.r += color.g*1.5;
//	color.g += color.b;
	return clamp(color,(0.0),(0.05));
}
//---------------------------------------------------------
vec3 computeColorB(float fv)
{
	vec3 color = vec3(vec3(CHANNELC)+TONE);
	color -= (fv);
//	color.r += color.g*1.5;
//	color.g += color.r;
	return clamp(color,(0.0),(0.67));
}
//---------------------------------------------------------
vec3 computeColorC(float fv)
{
	vec3 color = vec3(vec3(CHANNELB)+TONE);
	color -= (fv);
//	color.r += color.b*1.5;
//	color.g += color.b;
	return clamp(color,(0.0),(0.333));
}
//---------------------------------------------------------
void main() 
{
...
// (wolfFaceEQ(p+vec3(7.0, 1.0, 5.25),TIME));

	vec3 color = vec3 (mandelbrotEQ(vec3(-(p.y+0.667),p.x,p.z),pow(sin(TIME*1.5),cos(TIME*0.05))));
	vec3 cc = vec3 (quadTorusEQ(vec3(color.x-0.5,color.y,pow(color.z,p.z)),log2(TIME*0.05)-cos(TIME*0.25)));
	vec3 ccc = computeColorA(dogFaceEQ(vec3(-sin(cc.x+p.y),cos(cc.y+p.x)-0.125,(cc.z)), pow(TIME,p.z)-1.0));
	ccc += computeColorB(dogFaceEQ(vec3(sin(cc.x-p.x),-cos(cc.y-p.y)-0.25,(-cc.z)), pow((TIME*0.5),p.y)-1.5));
	ccc += computeColorC(dogFaceEQ(vec3(cos(cc.x-p.y),sin(cc.y+p.x),log(cc.z*TIME)+2.5),pow(-sin(TIME*0.05),TIME)));
