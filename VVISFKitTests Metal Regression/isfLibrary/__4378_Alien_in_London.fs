/*{
    ...
    "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/4slGRn by iq.  Trying to integrate a raymarched thing with a 2d background",
    "IMPORTED": {
    },
    "INPUTS": [
        {
...
vec2 monster( vec3 p )
{
	p.x += 1.4*sin( 0.55*Animatie + p.y );
	p.z += 1.4*sin( 0.75*Animatie + p.y );
	
	p.x += sin( (3.2-simpeler)*more*p.y + 0.0*Animatie );
    p.z += sin( (2.0-simpeler)*p.x  + 1.0*p.y - 0.0*Animatie );

	p.xz *= 3.0-0.25*(p.y+0.55);//1.5 - 1.0*sin(0.2*TIME);

	p.y -= 0.4 + 5.0*sin(0.2*Animatie);

	float r = 4.0 + Ribbel*sin(10.0*p.y);
    return vec2( length(p*more2) - r, 1.0 );
}
...
void main() {
    ...
    // camera
    vec3 ro = vec3( 0.0, een, twee );
    vec3 ww = normalize(vec3(0.0,1.0,0.0) - ro);
    vec3 uu = normalize(cross( vec3(0.0,1.0,0.0), ww ));
    vec3 vv = normalize(cross(ww,uu));
    vec3 rd = normalize( p.x*uu + p.y*vv + 1.5*ww );
    vec3 col = IMG_NORM_PIXEL(iChannel0,mod(q,1.0)).xyz;
    // raymarch
    ...
    if( tmat.y>0.5 )
    {
        ...
        if( tmat.y<1.5 )
		{
            ...
            vec3 mcol = IMG_NORM_PIXEL(iChannel0,mod(0.5 + 0.5*nor.xy*vec2(1.0,1.0) + grootte*pos.xy,1.0)).xyz;
            col *= 1.5*mcol;
