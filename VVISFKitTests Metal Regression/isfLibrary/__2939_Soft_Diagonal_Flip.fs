...
void main()	{
	...
	vec2		p1 = vec2(0.0,intercept);
	vec2		p2 = (abs(angle)==0.5) ? vec2(0.0,sign(angle)*10.0) : vec2(1.0,intercept+tan(angle*pi));
	float		val = distancePtToLine(p1,p2,loc);

	if (abs(val) < lineWidth)	{
		vec4	pix1 = IMG_NORM_PIXEL(inputImage,loc);
		vec4	pix2 = IMG_NORM_PIXEL(inputImage,1.0-loc);
		...
		returnMe = IMG_NORM_PIXEL(inputImage,1.0-loc);
	}
