/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https://www.shadertoy.com/view/MdBGRh by bysse.  Nightsky using simplex noise.",
  "INPUTS" : [
    {
      "NAME" : "iMouse",
      "TYPE" : "point2D"
...
vec3 SUN = normalize(vec3(SUNRISE, -1.0, 0.0));
vec3 MOON = normalize(vec3(moon, 0.4, 0.3));


float snoise(vec3 v);

float clouds(vec3 p) {
	vec3 q = p + vec3(-0.1, sin(p.y)*0.25, 1.0)*(10.0+TIME);
	float v = 0.0;
...
void main() {
	...
	mat3 rot = look(mouse / RENDERSIZE.xy);
