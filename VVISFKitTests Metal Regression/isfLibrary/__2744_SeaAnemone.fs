...
vec3 filter() {
	vec2 delta = 1.0 / RENDERSIZE;
	
	vec3 val = IMG_NORM_PIXEL(buffer, isf_FragNormCoord.xy).xyz;


	vec3 l = IMG_NORM_PIXEL(buffer, isf_FragNormCoord.xy + vec2(0.04, delta.y)).xyz;
	vec3 r = IMG_NORM_PIXEL(buffer, isf_FragNormCoord.xy - vec2(0.04, delta.y)).xyz;
	vec3 u = IMG_NORM_PIXEL(buffer, isf_FragNormCoord.xy + vec2(delta.x, 0.01)).xyz;
	vec3 d = IMG_NORM_PIXEL(buffer, isf_FragNormCoord.xy - vec2(delta.x, 0.01)).xyz;

	vec3 n = IMG_NORM_PIXEL(noise, fract(isf_FragNormCoord.xy * 8. + TIME )).xyz - 0.5;
...
vec2 DE(vec3 pos) {
	...
	pos = pos * rotationMatrix(normalize(vec3(0., 0., 1.)), 0.75 + TIME/10. + 0.67 * sin(pos.z) );
	...
	float a = hex(pos, vec2(0.19 * repeat,  1.0/2.));
	float b = hex(pos, vec2(0.81 * 0.19 * repeat, 1.0));
...
vec3 raycast() {
	...
	for(int i = 0; i < (MAX_ITER); i++) {
		...
		thresh = exp(-1.88) * exp(t/Multiply);
	...
	float shade = dot(gradient(point, 0.2 ), -ray);
	float ao = 1. -  float(iter) / float(MAX_ITER);
	
	vec3 color = vec3(0.);
	
	if ( hit )
		color = palette(point.z/2.5) * sqrt(ao);
	return color;
