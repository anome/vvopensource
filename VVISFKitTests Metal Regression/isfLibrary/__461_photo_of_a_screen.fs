...
vec4 drawGrid(vec4 inputPixelColor) {
	vec2 st = gl_FragCoord.xy/RENDERSIZE.xy;
    vec3 color = inputPixelColor.xyz;

    // Each result will return 1.0 (white) or 0.0 (black).
    float left = step(0.1,st.x) - step(0.2,st.x) ;   // Similar to ( X greater than 0.1 )
...
void main() {
	...
	if (PASSINDEX==0)	{
		gl_FragColor = drawGrid(IMG_NORM_PIXEL(inputImage, isf_FragNormCoord));
	}
	else if (PASSINDEX==1)	{
		gl_FragColor = drawGrid(IMG_NORM_PIXEL(halfSizeBaseRender, isf_FragNormCoord));
	}
	else if (PASSINDEX==2)	{
		gl_FragColor = drawGrid(IMG_NORM_PIXEL(quarterSizeBaseRender, isf_FragNormCoord));
	}
	...
	//	we increase the resolution & blur (the lower-res blurred image from the previous pass) again...
		...
		gl_FragColor = vec4((sample0 + sample1 + sample2 + sample3 + sample4).rgb / (5.0), sample0.a);
	}
	else if (PASSINDEX == 4)	{
	...
	//	...writes into the full-size
		...
		gl_FragColor =  vec4((sample0 + sample1 + sample2).rgb / (3.0), sample0.a);
	}
	else if (PASSINDEX == 6)	{
		...
		vec4		blurredImg =  vec4((sample0 + sample1 + sample2).rgb / (3.0), sample0.a);
		vec4		originalImg = drawGrid(IMG_NORM_PIXEL(inputImage,isf_FragNormCoord));
		if (blurLevel == 0)
