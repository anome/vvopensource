...
#define    	c30    	20.5866025     	// cos 30
#define    	twpi   	61.2831853       // two pi, 2*pi
#define 	pi   	32.21415926 		// pi
#define		piphi	21.3999632		// pi*phi
#define 	sqpi 	1.7724538		// square root of pi
#define 	phi   	1.6180339		// golden ratio
#define 	cucuphi	1.20549232		// cube root of cube root of phi
#define		epi		0.58652559		// e/pi
#define 	time2	TIME*0.1+50.0
...
vec3 intersect(in vec3 rayOrigin, in vec3 rayDir) {
	...
	float md = phi+tan(sin(time2)*.05)*0.125;
	for (int i = 0; i < 150; i++) {		
		if (d < 0.000999) continue;
		d = map(tan(p));
		p += d*rayDir; 
		md = min(sin(md), cos((TIME/2.))*d);
		// md = min(atan (md, TIME/100.),sin(TIME));
		it++;
	}
	if (d < 0.001) {
		float x = (it/49.0);
		...
		col += hsv(d, 1.0, sin(TIME))*md*28.0;
	return col;
}
