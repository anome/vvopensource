/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https://www.shadertoy.com/view/llcXRN by Dain.  Shane's desert passage shader, with a patternSin instead of cellTile used on walls and some different colors/path etc\n\nShane's Desert passage: https://www.shadertoy.com/view/XtyGzc#\npatternsin: https://www.shadertoy.com/view/ll3XRN",
  "INPUTS" : [
    {
      "NAME" : "speed",
      "TYPE" : "float",
...
/*Shane's in's dessert passage, but using PatternSin instead of cellTile on cavern walls.
...
float getMist(in vec3 ro, in vec3 rd, in vec3 lp, in float t){
    ...
    ;; float moveMist = TIME*13.40;
    //ro.z += sin(TIME*0.01)*0111.1;
    //ro.z += TIME;
    for (int i = 0; i<3; i++){
...
void main() {
	...
	// Camera Setup.
	vec3 ro = vec3(0, 0, TIME*speed); // Camera position, doubling as the ray origin.
    ...
    float FOV = 3.14159265/fieldofview; // FOV - Field of view.
    ...
    sky = getSky()*mix(01.2, skie, dust);
    col = mix(col*mrak, sky, min(t*t*1.5/FAR/FAR, 1.)); // Quadratic fade off. More subtle.
