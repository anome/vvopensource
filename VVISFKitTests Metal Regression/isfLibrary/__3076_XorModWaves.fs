...
////////////////////////////////////////////////////////////
// XorModWaves  by mojovideotech
//
// Creative Commons Attribution-NonCommercial-ShareAlike 3.0
////////////////////////////////////////////////////////////


float xor(float a, float b) { return mod((a - b), 1.0); }

float zor(float a, float b) { return pow((a - b), xor(a,b)); }
...
void main( void ) {
	...
	float radius = mod(1.0 - abs(offset.x*fract(T*0.05) - 1.0), 1.0);
	...
	float clrr = zor(clr, smoothstep(mod(invy, scale*1.5), mod(-invx, scale), scale));	
	vec3 col = vec3(clr,clrr,cos(clr));
   	if (flip) col = col.gbr;
	vec3 cc = col;
	if (flop) cc = cc.brg;
