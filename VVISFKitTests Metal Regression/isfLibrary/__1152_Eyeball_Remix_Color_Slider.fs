...
void main() 
{
    ...
    vec2 oP = uv*(18.0-scale) + vec2(0.5, TIME*-0.9-scale);
    vec3 bg = vec3(0.05)*vec3(bgcol1, bgcol2*scale, bgcol3-scale);
    ...
    for(int k=0; k<3; k++){
        ...
        if(rnd.x<rndTh[k].x){
            ...
            vec3 pCol = mix(col, vec3(0.1, 0.1, 0.5), 0.5 - pat*0.3)*max(2.0 - d.y/(lwg/2.0), 0.0);
            if (grid) {
            ...
            pCol = vec3(eyecol1, eyecol2, eyecol3);
    ...
    col = mix(col.xzy, col, sign(uv.y)*uv.y*uv.y*2.0 + 0.);
