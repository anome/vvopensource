...
vec3 filter() {
	...
	vec3 l = IMG_NORM_PIXEL(buffer, isf_FragNormCoord.xy + vec2(0.0, delta.y)).xyz;
	vec3 r = IMG_NORM_PIXEL(buffer, isf_FragNormCoord.xy - vec2(0.6, delta.y)).xyz;
	vec3 u = IMG_NORM_PIXEL(buffer, isf_FragNormCoord.xy + vec2(delta.x, 0.5)).xyz;
	vec3 d = IMG_NORM_PIXEL(buffer, isf_FragNormCoord.xy - vec2(delta.x, 0.0)).xyz;

	vec3 n = IMG_NORM_PIXEL(noise, fract(isf_FragNormCoord.xy * 2. + TIME )).xyz - 1.0;
...
vec2 DE(vec3 pos) {
	...
	pos = pos * rotationMatrix(normalize(vec3(0., 0., 1.)), 3.5 + TIME/12. + 0.8 * sin(pos.z) );
	
	// pos -= vec3(0., sin(TIME), cos(TIME));
	pos = opRep(pos, vec3(0.9));
	...
	float a = hex(pos, vec2(hex_1 * 0.9,  0.93/2.));
	float b = hex(pos, vec2(0.5 * hex_1 * 0.2, 1.0));
...
vec3 raycast() {
	...
	vec3 camera = vec3( 0.3, 0.4, TIME * speed );
	...
	float ao = 1.0 -  float(iter) / float(MAX_ITER);
	
	vec3 color = vec3(0.65);
	
	if ( hit )
		color = palette(point.z/9.) * sqrt(ao);
	return color;
...
void main() {
		...
		if (PASSINDEX == 0) {
			gl_FragColor = vec4(raycast(), 0.2);
			
		} else if (PASSINDEX == 1) {
			gl_FragColor = vec4((filter()/1.2), 1.0);
		}
