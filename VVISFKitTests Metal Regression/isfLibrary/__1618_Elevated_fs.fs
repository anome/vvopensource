/*
{
  ...
  "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/MdX3Rr by iq.  A raymarched version of my demo Elevated from 2009",
  "IMPORTED": [
    {
      "NAME": "iChannel0",
      "PATH": "tex12.png"
...
vec3 noised( in vec2 x )
{
	...
	float a = IMG_NORM_PIXEL(iChannel0,mod((p+vec2(0.5,0.5))/256.0,1.0),-100.0).x;
	float b = IMG_NORM_PIXEL(iChannel0,mod((p+vec2(1.5,0.5))/256.0,1.0),-100.0).x;
	float c = IMG_NORM_PIXEL(iChannel0,mod((p+vec2(0.5,1.5))/256.0,1.0),-100.0).x;
	float d = IMG_NORM_PIXEL(iChannel0,mod((p+vec2(1.5,1.5))/256.0,1.0),-100.0).x;
...
{
    float f = 0.0;
    f += 0.5000*IMG_NORM_PIXEL(iChannel0,mod(p/256.0,1.0)).x; p = m2*p*2.02;
    f += 0.2500*IMG_NORM_PIXEL(iChannel0,mod(p/256.0,1.0)).x; p = m2*p*2.03;
    f += 0.1250*IMG_NORM_PIXEL(iChannel0,mod(p/256.0,1.0)).x; p = m2*p*2.01;
    f += 0.0625*IMG_NORM_PIXEL(iChannel0,mod(p/256.0,1.0)).x;
    return f/0.9375;
}
...
vec3 render( in vec3 ro, in vec3 rd )
{
	...
	else
	{
        ...
        // rock
		float r = IMG_NORM_PIXEL(iChannel0,mod(7.0*pos.xz/256.0,1.0)).x;
        col = (r*0.25+0.75)*0.9*mix( vec3(0.08,0.05,0.03), vec3(0.10,0.09,0.08), IMG_NORM_PIXEL(iChannel0,mod(0.00007*vec2(pos.x,pos.y*48.0),1.0)).x );
...
void main()
{
	...
	vec2 s = xy*vec2(RENDERSIZE.x/RENDERSIZE.y,1.0);

    float time = TIME*0.15 + 0.3 + 4.0*iMouse.x/RENDERSIZE.x;
