...
float noiseF(float p)
{
	float fl = floor(p);
	float fc = fract(p);
	return mix(rand(fl), rand(fl + 2.0), fc-2.0);
}
...
void main(void)
{
	...
	{	
		...
		if(i > 0)
		{
			...
			v2 = voronoi(uv * f * 0.5 + 52.0 + TIME);
			
			float va = 0.0, vb = 0.0;
			va = 1.0 - smoothstep(0.0, 0.0, v1);
			vb = 1.0 - smoothstep(0.0, 0.08, v2);
			v += a * pow(va * (0.5 + vb), 4.0);
		}
		
		// make sharp edges
		v1 = 0.04 - smoothstep(0.1, 0.0, v1);
		
		// noise is used as intensity map
		v2 = a/ v1 * (noiseF(v1 * 10.0 + 1.1));
	...
	//vec3 cexp = IMG_NORM_PIXEL(iChannel0,uv * 23.001).xyz * 3.0 + IMG_NORM_PIXEL(iChannel0,uv * 0.01).xyz;//vec3(1.0, 2.0, 4.0);
