...
void main(void)
{
    ...
    // pour avoir une rotation sur le plan vertical
    //vec3 rd = normalize(vec3(asp*(pixel.y)/2.0, pixel.x, -2.0));
    
     // pour avoir une rotation sur le plan horizontal
  vec3 rd = normalize(vec3(asp*pixel.x, pixel.y, -2.0));
    vec3 ro = vec3(HorizontalShift, VerticalShift, SIZE);
		
	// rotate view
    float ax = TWOPI * ViewRotate;

//to obtain rotation around horizontal
 //rd = rotateY(rd, ax);
 //ro = rotateY(ro, ax);
