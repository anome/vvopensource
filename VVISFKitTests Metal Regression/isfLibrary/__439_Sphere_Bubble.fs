/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https://www.shadertoy.com/view/ldByRK by riginosk.  Sphere blob",
  "INPUTS" : [
    {
      "NAME" : "iMouse",
      "TYPE" : "point2D"
...
void main() {
    ...
    vec4 col = vec4(0.0125,0.,0.025,0.0);
    /*
    for (float j = 1.;j<NUM_RAYS+1.;j++)
    {
		...
		if ( rz >= FAR)continue;
    	vec3 pos = ro+rz*rd;
    	col.rgb = max(col.rgb,vmarch(pos,rd,j, bro));
    }
    */
    ro = bro;
    rd = brd;
    ...
    if (sph.x > 0.)
    {
        ...
        col.rgb += (0.1*nz*nz* vec3(0.12,0.12,.5) + 0.05*nz2*nz2*vec3(0.55,0.2,.55))*0.8;
        col.a += (0.1*nz*nz + 0.05*nz2*nz2)*0.8;
    }
