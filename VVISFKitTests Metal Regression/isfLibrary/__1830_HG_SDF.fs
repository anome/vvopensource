/*{
  ...
  "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/Xs3GRB by tomkh.  Simple test/port of <a href=\"http://mercury.sexy/hg_sdf\"  class=\"regular\" target=\"_blank\">Mercury's SDF library</a> to WebGL.<br/>To make it clear: the library is done by Mercury team for OpenGL 4+, not me, and this is just an unofficial port.",
  "INPUTS": [
    {
      "NAME": "iMouse",
      "TYPE": "point2D"
...
// GLSL library of signed distance functions by Mercury crew 
// a messy conversion for ISF via ShaderToy by mojovideotech
...
const float fog_density = .01;

const float cam_dist = 8.;
...
vec3 sky_color(vec3 ray_dir, vec3 light_dir)
{
   ...
   float d2 = light_dir.y*.7+.2;
   vec3 base_col;
   base_col = mix(vec3(.25),vec3((ray_dir.y<0.)?0.:1.),abs(ray_dir.y));
   return base_col*d2;
}
/*
...
vec3 shade(vec3 ray_start, vec3 ray_dir, vec3 light_dir, vec4 hit)
{
   ...
   if (hit.w == 0.0) {
      ...
      vec3 base_color = texture(hit.xyz,ray_len/RENDERSIZE.y).xyz;
      color = mix(vec3(0.,.1,.3),vec3(0.5,0.3,.9),diffuse)*base_color +
         spec*vec3(1.,1.,.9);
...
void main()
{
   ...
   vec3 light_dir = normalize(vec3(.5, 1.0, -0.25));
   
   // Simple model-view matrix:
   float ang, si, co;
   ang = (iMouse.x - RENDERSIZE.x*.5) * -.003 - TIME*.125;
   ...
   mat4 cam_mat = mat4(
     ...
     -si, 0., co, 0.,
      0., 0., 0., 1.);
   ang = (iMouse.y - RENDERSIZE.y*.5) * -.003 + TIME*.25 ;
   si = sin(ang); co = cos(ang);
...
float fField(vec3 p)
{
   ...
   p.xz = -p.xz;
   vec2 q = pMirrorOctant(p.xz,vec2(2.));
#endif
 //  float dodec = fDodecahedron(p-vec3(-2.25,.5,-1.),.7);
   float box = fBox(p-vec3(0,0,0),vec3(1.25));
   float sphere = length(p-vec3(0.5,0.5,0.5125))-1.25;
   float d;
   float r = 0.4;
   float n = 3.;
