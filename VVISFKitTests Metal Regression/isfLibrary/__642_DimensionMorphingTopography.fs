...
float	scene(vec3 p) {
    ...
    ii=0.0;
    m = r2;
    float aa = TIME*rate*0.008; //added zeroes
    p.z+=6.0;
    float tr = TIME*roto*0.01; //*.25 for smoothness
    ...
    for(float	i = -1.0; i < 24.0; ++i) {
        ...
        if (mod(i, 3.0) == 0.0) {
            ...
            p.zx*=ma;
	        p.xz = abs(p.xz)-0.125; //add reactive freq range
	       	p.y -= 0.2;
        }
...
vec3 evaluateLight(in vec3 pos) {
    vec3 lightCol = vec3(blend.x,length(blend),blend.y)+sin(tc); // add basspresence
