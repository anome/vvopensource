...
void main() {
   ...
   // int rayCount = 8;
    ...
    // center
    vec2 c = (gl_FragCoord.xy - vec2(RENDERSIZE) * 0.5) / RENDERSIZE.y;
    ...
    // output
    gl_FragColor = vec4(mix(vec3(1.0,1.0,1.0)*LightDark,mix(color2, color1, mask),distRound),1.0);
}
