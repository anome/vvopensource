/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https://www.shadertoy.com/view/MsGczV by AntoineC.  Simple animation using a gear polar function discovered mostly by chance.",
  "INPUTS" : [
    {
      "NAME" : "gear",
      "TYPE" : "float",
...
void DrawGear(inout vec3 color, vec2 uv, Gear g, float eps)
{
    ...
    float s = 1.0 - gear*GearShadow(uv, g);
...
void main() {
    ...
    float t = speed*TIME;
    vec2 uv = 2.0*(gl_FragCoord.xy - 0.5*RENDERSIZE.xy)/RENDERSIZE.y;
    float eps = 2.0/RENDERSIZE.y;
    // Scene parameters;
	vec3 base = vec3(colorofall);
    float count = count;
