...
vec2 DE(in vec3 p)
{
	...
	float d = sdBox(p.yxz, _shape.xxy);
	// d = min(d, sdBox(p.yxz - 1., _shape.xxy/2.));
	
	vec3 rep = vec3(_repeat, 0., _repeat);
	d = min(d, sdBox(p.yxz , _shape.yyx));
	d = min(d, sdBox(p.yxz - rep  * 0.5, _shape.yyx));

	d = min(d, sdBox(p.yxz - rep, _shape.yyx));
	d = min(d, sdBox(p.yxz - 1.5 * rep, _shape.yyx));
...
float edge( in vec3 pos )
{
	vec3 eps = vec3( _edge_size, 0.0, 0.0 );
	float c = DE(pos).x;

	float sobel =
	    DE(pos+eps.xyy).x + DE(pos-eps.xyy).x +
	    DE(pos+eps.yxy).x + DE(pos-eps.yxy).x +
	    DE(pos+eps.yyx).x + DE(pos-eps.yyx).x  - 6.0 * c ;
	return abs(sobel / eps.x);
}
...
vec3 intersect(in vec3 rayOrigin, in vec3 rayDir)
{
	...
	float shade = 1.0 - iter;
	if ( hit) {

		color = vec3(1. - x)  ; //palette(x + d.y);
		color += vec3(0.,1.,0.) * edge(p);
	} else {
		color = vec3(0.);
	}
