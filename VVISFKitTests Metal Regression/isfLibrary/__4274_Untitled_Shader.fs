...
vec4 hsvGlitch(vec2 uv){
    	vec4		inputPixelColor;

	inputPixelColor = IMG_THIS_PIXEL(inputImage);

    vec3 hsv = rgb2hsv(inputPixelColor.rgb);
    
    float angle = hsv.x + atan(uv.y, uv.x) + TIME*0.1;
    mat2 RotationMatrix = mat2( cos( angle ), -sin( angle ), sin( angle ),  cos( angle ));
    vec2 aa =  uv + RotationMatrix * vec2(log(MAX(SIN01(TIME*0.7)-0.2, 0.)*0.2 + 1.  ),0) * hsv.y;
    vec3 col = IMG_PIXEL(inputImage,aa).rgb;
...
void main()	{
	...
	gl_FragColor = hsvGlitch(isf_FragNormCoord.xy);
}
