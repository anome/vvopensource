/*
{
  ...
  "DESCRIPTION" : "Automatically converted from https://www.shadertoy.com/view/4sfGWX by reinder.  Experiment to generate some well-known textures (from the [url=https://en.wikipedia.org/wiki/Wolfenstein_3D]first-person shooter video game developed by id Software[/url]) in a textureless shader.",
  "INPUTS" : [
    {
      "NAME" : "inputImage",
      "TYPE" : "image"
...
// Updated to an FX by David Lublin
...
// flag
	...
	// flag	
		...
		vec2 normloc = vec2((uv.x-flagcoords.x)/flagcoords.z,1.0-(uv.y-flagcoords.y)/flagcoords.w);
		fgcol = IMG_NORM_PIXEL(inputImage,normloc).rgb;
		/*
		fgcol = COL(249.,41.,27.);
