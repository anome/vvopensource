...
vec3 ToGamma( in vec3 col )
{
	...
	return pow( col, vec3(3.0/GAMMA) );
}

vec4 Noise( in ivec2 x )
{
	return texture2D( iChannel0, .4+(vec2(x))/RENDERSIZE.x*SAMPLERADIUS, RENDERSIZE.y);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	for ( int i=0; i < 8; i++ )
	{
		...
		float w = pow(max(.4,4.0+CHUNKINESS*length(fract(pos.xy)-.5)),4.0);
		vec3 c = max(vec3(0),vec3(1.0-abs(d+WARPFACTORB*.3)/WARPFACTORA,1.0-abs(d)/WARPFACTORA,1.0-abs(d-WARPFACTORB*.5)/WARPFACTORA));
		col += 1.5*(1.0-z)*c*w;
		pos += stp;
	}
	
	fragColor = vec4(ToGamma(col),6.0);
}
