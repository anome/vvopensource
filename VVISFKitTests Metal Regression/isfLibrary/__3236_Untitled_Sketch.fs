/*
{
  ...
  "INPUTS" : [
    ...
    {
      "NAME" : "light_lfo_speed",
      "TYPE" : "float",
      "MAX" : 3,
      "DEFAULT" : 1,
      "LABEL" : "Light LFO Speed",
      "MIN" : 0.0
    }
  ],
  "PASSES" : [
...
vec3 GDFVectors(int idx) {
	vec3 Vectors[19];
	Vectors[0] = vec3(normalize(vec3(1.0, 0.0, 0.0)));
	Vectors[1] = vec3(normalize(vec3(0,1,0)));
	Vectors[2] = vec3(normalize(vec3(0,0,1)));

	Vectors[3] = vec3(normalize(vec3(1,1,1)));
	Vectors[4] = vec3(normalize(vec3(-1,1,1)));
	Vectors[5] = vec3(normalize(vec3(1,-1,1)));
	Vectors[6] = vec3(normalize(vec3(1,1,-1)));

	Vectors[7] = vec3(normalize(vec3(0.0, 1.0, PHI+1.0)));
	Vectors[8] = vec3(normalize(vec3(0.0, -1.0, PHI+1.0)));
	Vectors[9] = vec3(normalize(vec3(PHI+1.0, 0, 1)));
	Vectors[10] = vec3(normalize(vec3(-PHI-1.0, 0, 1)));
	Vectors[11] = vec3(normalize(vec3(1, PHI+1.0, 0)));
	Vectors[12] = vec3(normalize(vec3(-1, PHI+1.0, 0)));

	Vectors[13] = vec3(normalize(vec3(0, PHI, 1)));
	Vectors[14] = vec3(normalize(vec3(0, -PHI, 1)));
	Vectors[15] = vec3(normalize(vec3(1, 0, PHI)));
	Vectors[16] = vec3(normalize(vec3(-1, 0, PHI)));
	Vectors[17] = vec3(normalize(vec3(PHI, 1, 0)));
	Vectors[18] = vec3(normalize(vec3(-PHI, 1, 0)));
...
// Ray Marching Forward Decalres
float RayMarch(vec3 ro, vec3 rd);
float GetLight(vec3 p);
vec3 GetNormal(vec3 p);

// Define the geometry of the scene here! 
float map(vec3 p) {
	float r = 0.3;
	float n = 9.0;

	float sphere = fSphere(p-vec3(0, 1.0+abs(sin(t))*2.0, 6.0),1.0);
	
	// Rotating Box
	vec3 bp = p;
	bp -= vec3(sin(t)*2.5, 1, 6); // translate
	pR(bp.xz, t); 				  // rotate
    float box = fBox(bp,vec3(1.0,1.0,1.0));
    
    float plane = p.y;
    
    float d = fOpUnionRound(sphere, box, 1.0);
    d = min(d, plane);
...
void main()	{
    ...
    vec3 ro = vec3(cam_x, cam_y, 0);
    vec3 rd = normalize(vec3(uv.x, uv.y, cam_z));
...
float GetLight(vec3 p) {
    ...
    lightPos.xz += vec2(sin(t*light_lfo_speed), cos(t*light_lfo_speed))*light_lfo_amp;
