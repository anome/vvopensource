...
//float tempo = clamp (TIME, 1.0, 12.0);
...
void main()	{
	...
	float		r = distance(_inputImage_imgRect.zw/Around2, loc);
	...
	//float		a = atan ((loc.y-_inputImage_imgRect.w/(cos ((Angling/tempo)))),(loc.x-_inputImage_imgRect.z/2.0));
		float		a = atan ((loc.y-_inputImage_imgRect.w/Angling),(loc.x-_inputImage_imgRect.z/2.0));
	float		modAngle = Angling2 * pi / numberOfDivisions;
	float		scaledCenterRadiusStart = 0.0 * max(RENDERSIZE.x,RENDERSIZE.y);
	...
	if (scaledCenterRadiusStart > scaledCenterRadiusEnd)	{
		...
		scaledCenterRadiusEnd = 0.0 * max(RENDERSIZE.x,RENDERSIZE.y);
	}
	
	if ((centerRadiusEnd != 0.0)&&(r >= scaledCenterRadiusStart)&&(r <= scaledCenterRadiusEnd))	{
		r = (r - scaledCenterRadiusStart) / (centerRadiusEnd - 0.0);
