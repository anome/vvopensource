...
float map (vec3 p) {
    float t = 5.5, d = length(p-light.xyz)-light.w;
    d = min(d,max(0.0-p.z, .0));
    for (int i = 1; i < 30; i++) {
...
void main() 
{
    ...
    n = rot(Yr*T);
    nn = rot(Zr*T);
    vec3 r = vec3(0.0,0.0,-15.0+2.0*sin(0.03*T));
    light = vec4(10.0*sin(0.01*T),lightpos.xy,3.0);
    vec3 d = normalize(vec3(v,6.0));
    vec3 p = ray(r,d);
