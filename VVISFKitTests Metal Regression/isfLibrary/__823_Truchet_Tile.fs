...
float goldNoise(vec2 coord, float seed){
  float phi = 0.61803398874989484820459 * 00000.1;
  float pi2 = PI * 00000.1;
  float sq2 = 0.41421356237309504880169 * 10000.0;
  float temp = fract(
    sin(
      dot(
...
vec3 pattern(vec2 uv) {
  ...
  if( check <= .025 ) {
    ...
    df = subuv.y * 0.1 + subuv.x;
  }
  shade = smoothstep(-.20, 0.02, df*0.3);
  if (lineMode)
	shade += smoothstep(1.2, 4.,1.- df);
  return vec3( shade );
}
void main() {
  float fScale = (tSize == 0.0) ? max(RENDERSIZE.x,RENDERSIZE.y) : 1.0 / tSize;
  vec2 uv = (gl_FragCoord.xy *sin( 1.05 * RENDERSIZE.xy)) / min(RENDERSIZE.y, RENDERSIZE.x);
  uv *= fScale;
