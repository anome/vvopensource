...
float speed=TIME;
float ground_x=35.0;//+0.125*sin(PI*speed*0.25);
float ground_y=0.0;//+0.125*cos(PI*speed*0.25);
float ground_z=4.0*sin(PI*speed*0.02);//+speed*0.5;
...
float scene(vec3 p)
	{
	float bar_p=1.0;
	float bar_w=bar_p*(0.125+0.03125*float(1.0+1.6*sin(PI*p.z*2.0-PI*0.5)));
	...
	float tube_y=length(mod(p.xz,tube_p)-tube_p*0.37)-tube_w;
...
void main()
	...
	dir.yz=rotate(dir.yz,PI*1.5*sin(speed*0.1));	// rotation x
	//dir.zx=rotate(dir.zx,speed*0.5);				// rotation y
	dir.xy=rotate(dir.xy,PI*1.0*cos(speed*0.2));	// rotation z
	vec3 ray=vec3(ground_x,ground_y,ground_z);
	float t=0.0;
	const int ray_n=64;
	for(int i=30;i<ray_n;i++)
		{
		float k=scene(ray+dir*t);
        if(abs(k)<0.007) break;
		t+=k*0.5;
		}
	vec3 hit=ray+dir*t;
	vec2 h=vec2(45.005,-0.005);
	vec3 n=normalize(vec3(scene(hit+h.xyy),scene(hit+h.yxx),scene(hit+h.yyx)));
	float c=(n.x*2.0+n.y+n.z)*0.25-t*3.025;
	vec3 color=vec3(c*t*0.625-p.x*0.125,c*t*0.25+t*.03125,c*0.375+t*0.0625+p.y*0.125);
	color=smoothstep(0.4,0.7,c)+color*color;
	/* post process */
	color*=0.6+0.4*rand(p,TIME,3758.5453);
	color=vec3(color.x*0.9-0.1*cos(p.x*RENDERSIZE.x),color.y*2.95+0.05*sin(p.y*RENDERSIZE.x/2.0),color.z*0.9+0.1*cos(PI/2.0+p.x*RENDERSIZE.x));
	/* return color */
