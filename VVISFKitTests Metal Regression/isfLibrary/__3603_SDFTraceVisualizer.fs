/*{
  ...
  "DESCRIPTION" : "Automatically converted from https://www.shadertoy.com/view/lslXD8 by paniq.  Testbed for tracing; probably only works on Chrome for Linux ;-)",
  "INPUTS" : [
    {
      "NAME" : "iMouse",
      "TYPE" : "point2D"
...
vec2 uv;
vec2 position;
float ScreenH = min(RENDERSIZE.x,RENDERSIZE.y);
...
void paint() {
	vec2 ms = ((iMouse.xy/RENDERSIZE.xy)*2.0-1.0) * aspect;
