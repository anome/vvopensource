...
void main()
{
	...
	if ((random.x < glitch_horizontal)&&(random.y < glitch_vertical))	{
		...
		vec4	srcPixel = IMG_NORM_PIXEL(inputImage,xy);
		 vec4	newPixel = IMG_NORM_PIXEL(one, xy);
		 srcPixel = srcPixel+newPixel;//clamp(srcPixel+newPixel, vec4(0.0), vec4(1.0));
		 srcPixel *= 0.50;

	
	gl_FragColor = vec4(mix(srcPixel.rgb, IMG_NORM_PIXEL(inputImage, xy).rgb, vec3(blend)), 1.0);
}
