/*
{
  ...
  "DESCRIPTION" : "Creates a splotch with optional left/right symmetry",
  "INPUTS" : [
    {
      "DEFAULT" : [
        0,
        0,
        0.021815943371908605,
        1
      ],
      "NAME" : "colorInput",
      "TYPE" : "color"
...
vec2 rotatePointNorm(vec2 pt, float rot)	{
	...
	float r = distance(vec2(0.00), returnMe);
	float a = atan((returnMe.y),(returnMe.x));

	returnMe.x = r * cos(a + 2.0 * pi * rot);
	returnMe.y = r * sin(a + 2.0 * pi * rot);
...
float splotchAmount(vec2 uv, float s)
{
	float nTime = splotchSize;
	float seed = s;

	// Animated radius parameter
	float tp = splotchSize - nTime;
	float radius = splotchSize - tp * tp * tp * tp;

	// Noise 1 - Radial curve
	vec2 fuv = vec2(-uv.x,uv.y);
	float freq = mix(1.2, 2.7, Random((seed*48923.23*(nTime))));
	float n1 = snoise(atan(vec2(uv.y, uv.x)) * freq + seed * 764.2174 * (nTime));
	float fn1 = snoise(atan(vec2(fuv.y, fuv.x)) * freq + seed * 764.2174 * (nTime));
	n1 = (uv.x > 0.0) ? mix(n1,fn1,rightSym) : mix(n1,fn1,leftSym);
	...
	float fn2 = snoise(fuv * 8.0 / radius + seed * 1481.28943);
	n2 = (uv.x > 0.0) ? mix(n2,fn2,rightSym) : mix(n2,fn2,leftSym);

	// Potential = radius + noise * radius ^ 3;
	float p = radius * (0.23 + radius * radius * (n1p * 0.9 + n2 * 0.07));

	// Antialiased thresholding
	float l = (uv.x > 0.0) ? mix(length(uv),length(fuv),rightSym) : mix(length(uv),length(fuv),leftSym);
...
void main()	{
	...
	uv = rotatePointNorm(uv,rotation);
	if (splotchSize > 1.0)
		uv *= splotchSize;
	float a = splotchAmount(uv,seedVal);
	//gl_FragColor = vec4(colorInput.rgb, colorInput.a * a);
	gl_FragColor = mix(bgColor,colorInput,a);
