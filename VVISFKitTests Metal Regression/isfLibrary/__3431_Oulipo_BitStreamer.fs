...
void main() {
    ...
    vec3 color = vec3(0.);
    color.r = pattern(st+off1,vel,0.5+((Variant*256.0))/RENDERSIZE.x);
    color.g = pattern(st,vel,0.5+(Variant*256.0)/RENDERSIZE.x);
    color.b = pattern(st-off2,vel,0.5+(Variant*256.0)/RENDERSIZE.x); 
