...
void main() {
	...
	float size = min(RENDERSIZE.x, RENDERSIZE.y);
	vec2 uv = gl_FragCoord.xy / size;
	// warp the UV -  sin curve to modulate with time
	uv += warp*(sin(6.3/20.)*2.-1.) * vec2(snoise2d(uv*4.),snoise2d(uv*2.));
	float sweeper = sin(uv.x*6.3/1.+TIME*hz) * 0.5 + 0.5;
	float sharpness = mix(0.9, cluster, sweeper);
	vec3 col = vec3(1.0+hue,hue+0.4, mix(0.1, 1.0, sweeper));
	col *= smoothstep(0.4, 0.5, pow(snoise3d( vec3(uv*10., TIME )),sharpness));
