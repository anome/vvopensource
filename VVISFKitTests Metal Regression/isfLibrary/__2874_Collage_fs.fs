...
// CALCULATE EDGES OF CURRENT CELL
	...
	if (cell_size == 0.0)	{
		...
		*/
		...
		float rows = floor(seed3/CellHeight);
		float cols = floor(seed4/CellWidth);
		...
		float translated = clamp(rand(vec2(region/count, seed2)),0.0, seed5 );
	...
	//	lastly if flips are allowed, randomly flip h
		if (allow_flips_h)	{
			float flipx = rand(vec2(translated, seed6));
			if (flipx > 0.5)	{
				xy.x = 1.0-xy.x;
			}
		}
		if (allow_flips_v)	{
			float flipy = rand(vec2(translated, seed6));
			if (flipy > 0.5)	{
				xy.y = 1.0-xy.y;
			}
		}
