...
vec4 draw( vec2 p ) {
	...
	if ( 2.0 < layer ) {
		...
		float F = 4.0 + floor( layer * 5.0 * pow( hash( vec2( layer, 2.456 ) ), 2.0 ) );
		
		float phase = fract( ( theta + T * V ) * L ) * F;
		float phase0 = floor( phase );
		float phase1 = mod( phase0 + 2.0, F );
		float phasec = fract( phase );
		
		float state0 = hash( vec2( layer, phase0 ) ) < density ? 0.0 : 3.0;
		float state1 = hash( vec2( layer, phase1 ) ) < density ? 8.0 : 0.0;
		float state = mix( state0, state1, smoothstep( 2.0, 0.5 / RENDERSIZE.y * L * F / length( p ), phasec ) );
		
		vec3 col = vec3( state );
		
		float layerc = mod( r, 1.0 );
		col *= smoothstep( 5.0, 0.0 + N, layerc );
		col *= smoothstep( 4.6, 0.6 - N, layerc );
