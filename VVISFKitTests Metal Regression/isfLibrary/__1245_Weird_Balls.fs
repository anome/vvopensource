...
float dist(vec3 p)//distance function
{
	...
	d=min(sdBox(p,vec3(boxy.x))-(1.0-boxy.x),d);
	for (int i=0; i<20; i++)
	{
		float fi = float(i); 
		if (fi>balls) {
		...
		float tof=seed/float(floor(balls))*fi;
		vec3 offs = vec3(
			cos(t*.7+tof*6.0),
			sin(t*.8+tof*4.0),
			sin(t*.9+tof*3.0));
		vec3 v = p+normalize(offs)*1.3;
		d = min(d,sdBox(v,vec3(boxy.y*.2))-(1.-boxy.y)*.3);
	}
...
vec3 background(vec3 p,vec3 d)//render background
{
	...
	return color*(noise(d*(.7+bgNoise))+bgNoise*pow(noise(d*4.0*(.7+bgNoise)),4.0));
...
vec3 object_material(vec3 p, vec3 d) //computes the material for the object
{
	...
	vec3 oldn=n; float nns = bumpiness.x; float nna = bumpiness.y;
	...
	float reflectance = shine+dot(d,n);
...
void main()
{
	...
	vec3 mouse = vec3(iMouse.xy*RENDERSIZE.xy - 0.5,0.5)/3000.;
