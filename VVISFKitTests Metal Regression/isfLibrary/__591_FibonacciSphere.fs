/*{
  ...
  "DESCRIPTION": "Automatically converted from http://glslsandbox.com/e#32455.0"
}*/

// FibonacciSphere by mojovideotech
// copied from glslsandbox.com/e#32455.0
...
float lattice(vec3 position, float k)
{
	...
	float inner_bound	=  sphere(position, 1.0);
	float outer_bound	=  sphere(position, 2.0-k*abs(sin(TIME)));
	
	return max(-inner_bound, max(outer_bound, distance_to_point-.025));
}


vec3 lattice_derivative(in vec3 p, float k)
{
	vec2 offset 	= vec2(0., .0005);

	vec3 derivative 	= vec3(10.);
	derivative.x 	= lattice(p+offset.yxx,k)-lattice(p-offset.yxx,k);
	derivative.y 	= lattice(p+offset.xyx,k)-lattice(p-offset.xyx,k);
	derivative.z 	= lattice(p+offset.xxy,k)-lattice(p-offset.xxy,k);
...
void main( void ) 
{
	...
	for(int i = 0; i < 50; i++)
	{
		...
		{
			float k = 1.0-float(i)/49.0;
			surface_distance 	= lattice(position,k);
	...
	vec3 gradient 		= lattice_derivative(position,0.0);
	vec3 normal		= normalize(gradient);
	
	
	vec3 light_position	= vec3(4., 10., -13.);
	vec3 light_direction	= normalize(light_position-position);
	
	float incident_light	= max(dot(normal, light_direction), 0.);
		
	
	vec4 result		= vec4((direction-normal),1.);
