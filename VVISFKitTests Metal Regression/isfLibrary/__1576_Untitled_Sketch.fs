...
int sprite(vec2 p)
	...
	if (clamp(p.x, 0.0, 16.0) == p.x && clamp(p.y, 0.0,11.0) == p.y)
		...
		float n = slice(int(s));
		k = (k - s*8.0)*3.0;
		if (int(mod(n/(pow(2.0,k)),4.0)) == 1) 		d += 1;
		if (int(mod(n/(pow(2.0,k+1.0)),4.0)) == 1) 	d += 1;
		if (int(mod(n/(pow(2.0,k+2.0)),4.0)) == 1) 	d += 1;
		}
	return d;
	}
...
void main()
	...
	float bh = 1.;// abs(60.0*sin(iChannelTime[0]*6.0)); // bounce height
	float hy = 0.0; // hero y
	float angle = 0.0; // should be optimized
	hero(col, -p, angle);
