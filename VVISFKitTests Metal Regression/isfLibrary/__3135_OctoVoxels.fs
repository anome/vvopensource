...
/*
    ...
    for(int y = 0; y < AA; ++y)
	    for(int x = 0; x < AA; ++x)
        {
    		...
    		time = TIME-sin(0.1*cos(TIME)) ;//- IMG_PIXEL(inputImage, ivec2(mod(gl_FragCoord.xy * float(AA) + vec2(x, y), 1024.)), 0).r * .02;
...
*/
    ...
    // Apply colour
    fragColor.rgb *= mix(vec3(colour_A.rgb),
                         mix(vec3(.1), vec3(colour_B.rgb), step(.66, cs)), step(.33,cs));
    ...
    fragColor.rgb *= pow(IMG_NORM_PIXEL(inputImage,mod(tu,1.0)).r, 1.5) * 1.1;
