...
// based on:
// http://glslsandbox.com/e#26508.3
...
vec2 cln(const vec2 z) {
  return vec2(log2(length(z*.667)), atan(z.y, z.x)); // +2k pi
}
...
vec3 hsv(float h, float s, float v) {
  return v * mix(
    vec3(0.5),
   clamp((abs(fract(h+vec3(3.25, 0.67, 2.25)/(0.333*cos(h/s)))*2.667)-sin(s+h)), 0.0, 1.0),
   // clamp((abs(fract(h+vec3(3.125, 1.25, 2.125)/(0.333*cos(h*s)))*3.0+(s/h))-0.95), 0.0, 1.0), 
    s);
}
