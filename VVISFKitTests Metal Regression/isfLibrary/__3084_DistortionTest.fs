...
void main() {
	...
	//p = vec2(0.0, T - log2(length(p.y))) + atan(p.y, p.x) / twpi;
	//p = vec2(0.0, T - log2(length(p.x))) + atan(p.y, p.x) / twpi;
	//p = vec2(0.0, T - log2(length(p.xy))) + atan(p.x, p.x) / twpi;
	//p = vec2(0.0, T - log2(length(p.xy))) + atan(p.y, p.y) / twpi; 
	//p = vec2(0.0, T - log2(length(p.xy))) + atan(p.y, p.x) / twpi; p = vec2(0.0, T - log2(length(p.xy))) + atan(p.y, p.x) / twpi; 
   	//p.y = ceil(p.y) - p.y;
   	//p.y = ceil(p.y) - p.x;
    p.x *= piphi;
