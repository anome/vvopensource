...
vec2 DE(in vec3 p)
{
	...
	p = p * RotationMatrix(vec3(1.0,1.0,0.0), _rotation);
	
	float color =  mod(3.0 * (RotationMatrix(vec3(1.0,1.0,0.0), -_rotation) * p).y , 1.) < 0.5 ? 1.0 : 0.0 ;
	
	return vec2(sdTorus(p.yxz, _shape), color);
}
...
vec3 intersect(in vec3 rayOrigin, in vec3 rayDir)
{
	...
	if ( hit == 1.0) {
		...
		if (_stripe)
			color = palette(x + d.y) * shade ;
		else
			color = palette(x ) * shade * d.y ;

	 	
	} else {
		color = 1.0 - palette(fbm(p.xy/10.));
		color /= 2.;
	}
