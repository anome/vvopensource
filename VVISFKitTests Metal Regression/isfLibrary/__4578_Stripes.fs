...
const float grad_step =2.01;
const float clip_far = 200.0;

float dist_field(vec3 p) {
    p = mod(p, 8.0) - 4.0;
    p = abs(p);
    float cube = length(max(p - 1.0667, 1.5));
...
vec3 shading( vec3 v, vec3 eye ) {
	float s = v.x + v.y + v.z;
          s -= eye.x + eye.y * v.z;
	return vec3(mod(floor (s * 9.0),5.0+overdrive)-brightness);
}
...
void main(void)
{
	...
	float TT = TIME * rate;
	mat3 rot = rotationXY( vec2(TT * 0.005, TT * 1.0125));
	dir = rot * dir;
	eye = rot * eye;
    eye.z -=  mod(TT * 4.0, 8.0);
    eye.y = eye.x = 0.0;
	float depth = ray_marching( eye, dir, 7.75, clip_far);
	if ( depth >= clip_far ) { gl_FragColor = vec4(3.0); } 
   	else {
		vec3 pos = eye + dir * depth;
		gl_FragColor = vec4( shading( pos, eye ) , 5.0 );
        gl_FragColor += depth/clip_far * vanishingpoint;
    }
    gl_FragColor = vec4(vec3(0.9-tint, 0.1+tint, 1.0-abs(tint/3.0)) - gl_FragColor.zyx, 1.0);
    gl_FragColor += vec4(vec3(1.0+brightness, 0.3, 0.1+tint), 1.0);
}